...
typedef enum { t_INT, t_STRING, t_DOUBLE } t_DataType;

class DataChangeObserver: public Observer, public CObject
{
    public:

	DataChangeObserver(GPTrack *track,DataValue *ptr,t_DataType typ,LPSTR _name,LPSTR _descr):
    ptr(ptr),
	Observer(track,t_DataChange,NULL),
	type(typ)
	{
	...
	BOOL ChangeDataValue(LPCSTR text)
	{
		switch(type)
		{
		   case t_INT:
