...
void Mak256SquareImage(CBitmap &jamBitmap,CBitmap &currentImg,int nWidth,int
nHeight,int hr,int vr)
{
       ...
       poldbmps = memdcA.SelectObject(&jamBitmap);
           ...
           for(int i=0;i<hr;i++)
           {
               for(int j=0;j<vr;j++)
                 ...
                 memdcB.StretchBlt( xpos,ypos, subImageSizeH,subImageSizeV,
&memdcA, 0,0, nWidth, nHeight,SRCCOPY );
                   }
           }
...
void CInternalView::OpenGLDrawScene(HDC hdc)
{
  ...
  Point3D **ptArray =
    (Point3D **)malloc(myobject->Verticies * sizeof(Point3D *));
      ...
      if ((X & 0xFF00) == 0x8000) {
        ...
        if (X > 0) {
          if (X > 0x80 && X < 0xFF) {
            ...
            ObjectData *Xdat =
              (ObjectData *)myobject->objectData->elementAt(scaleIDx);
            X = Xdat->GetValue();
            X = -X;
          } else {
  ...
  if (mytrack->showFilledObjects && myobject->polygonPoints != NULL) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (transparent) {
        ...
        glColor4f(
          1.0f, 1.0f, 1.0f,
          alpha);// note: because of decal mode only alfa is important here
      }
      ...
      for (int i = 0; i < ver->points; i++) {
        ...
        float hr = static_cast<float>(ver->m_HorizontalRepeat) / 256.0f;
        float vr = static_cast<float>(ver->m_VerticalRepeat) / 256.0f;
  ...
  if (mytrack->showPoints) {
    ...
    for (int i = 0; i < myobject->Verticies; i++) {
      ...
      // glVertex3d( ((double)(ptArray[pt2]->x)*SC),
      // ((double)(ptArray[pt2]->y)*SC), ((double)(ptArray[pt2]->z)*SC));
    }
    glEnd();
  }
