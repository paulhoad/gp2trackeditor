...
BOOL CZoomFactor::SetLimits(const UINT max_in, const UINT max_out)
{
  ...
  if (0 == max_in || 0 == max_out) return FALSE;
  ...
  if (!CanMagnify()) MagnifyTo(m_MaxIn);
  if (!CanShrink()) ShrinkTo(m_MaxOut);
  ...
  if (!m_bSpecific) {
    ...
    if (m_Factor < 1.0f)// Denominator > Numerator
    {
      ...
      if (JustBelowIntegerZoom(m_Factor, 0.0001f))
        ...
        // so close to being an integer anyway.  Think of original m_Factor
        // == 6.9999
        m_Denominator = max(1, min(m_MaxOut, m_Denominator + 1));
      m_Numerator = 1;
    } else {
...
{
  if (numerator < 1 || numerator > m_MaxIn) return FALSE;
...
{
  if (denominator < 1 || denominator > m_MaxOut) return FALSE;
...
BOOL CZoomFactor::SetFactor(const float factor)
{
  ...
  if (factor > (float)m_MaxIn || factor < (1.0f / (float)m_MaxOut))
    return FALSE;
...
CSize CZoomFactor::MagnifyArea(const CSize& sizWindow, const CSize& sizAOI, const CSize& sizDoc)
{
  ...
  if (sizDoc.cx < 1 || sizDoc.cy < 1 || sizWindow.cx < 1 || sizWindow.cy < 1 || sizAOI.cx < 1 || sizAOI.cy < 1)
    return CSize(-1, -1);
  ...
  if (m_Factor < (1.0f / (float)m_MaxOut)) m_Factor = (1.0f / (float)m_MaxOut);
  if (m_Factor > (float)m_MaxIn) m_Factor = (float)m_MaxIn;
...
// Is the given zoom factor roughly equivalent to an integer ratio? e.g. 1:6.001
// or 3.002:1
