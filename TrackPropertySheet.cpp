...
int CTrackPropertySheet::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	if (!m_TrackProperty.CreateEx(WS_EX_CLIENTEDGE,"SysListView32","", WS_VISIBLE | WS_CHILD | WS_BORDER | LVS_REPORT | LVS_EDITLABELS,10,10,100,100,GetSafeHwnd(),(HMENU)ID_TRACK_PROPERTY))
       return -1;
...
void CTrackPropertySheet::UpdateInfo()
{
	...
	int colDesc = m_TrackProperty.InsertColumn(2,"Description",LVCFMT_LEFT,500,2);
}
...
void CTrackPropertySheet::LoadTable(Observer *obj)
{
	if (obj->isSingleVariable())
	{
	 ...
	 //InsertTableString( 0,2,(LPSTR)(LPCTSTR)(svobj->GetDescription()),(LPARAM)svobj);
...
void CTrackPropertySheet::OnSize(UINT nType, int cx, int cy) 
{
    ...
    m_TrackProperty.SetWindowPos(NULL,rc.left + border, rc.top + border,
