...
void CPaletteChooser::OnDrawItem(int nIDCtl, LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	...
	else if (nIDCtl == IDC_SELECTED_COLOR)
	{
		...
		CDC* pDC = CDC::FromHandle(lpDrawItemStruct->hDC);
        CRect rcItem(lpDrawItemStruct->rcItem);
		CBrush *brush = new CBrush(RGB(palette[selectedColor].r,palette[selectedColor].g,palette[selectedColor].b));
	    CBrush *brushold = pDC->SelectObject(brush);
		pDC->SelectStockObject(BLACK_PEN);
...
LRESULT CPaletteChooser::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
