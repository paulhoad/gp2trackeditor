...
typedef enum { t_Air,t_Cockpit, t_ViewCamera } t_GLView;
...
public:
	...
	void getLookPoint();
	void InitView(int from,int to);

	void DrawScenery(GPTrack *track);

	void DrawRibbon3DGL(GPTrack *track,double xpos,double ypos,double zpos,
				        double x1,double y1,double z1,
				double nx1,double ny1,double nz1,
				double x2,double y2,double z2,
				double nx2,double ny2,double nz2,int textureId);

	void OpenGLDrawPitlane(HDC hdc);
	...
	static unsigned char * TextureMap(CDC *pDC,CBitmap &cbmp,unsigned char *lpBits,BOOL transparent);
    static void LoadJamTexture(GPTrack *mytrack,CBitmap &output,int idx,int hres,int vres,int *width,int *height);

	void InitDisplayList(HDC hdc);
	...
	void drawTextureSides(TrackSection *t,GPTrack *track);
    void drawSingleSectionSide(TrackSection *t1,int beginOffset,int length,int side);
    void drawSingleSide(TrackSection *t1,int beginOffset,int length,int side);
...
protected:
	...
	afx_msg void OnAirview();
	afx_msg void OnUpdateAirview(CCmdUI* pCmdUI);
	afx_msg void OnViewCockpit();
	afx_msg void OnUpdateViewCockpit(CCmdUI* pCmdUI);
