...
public:
  ...
  TrackObject(GPTrack *track, TrackSection *owner, int lengthfromstart, int _code, int myoffset)
    : TrackCmd(track, owner, 0x80, myoffset, "TrackObject"), selected(FALSE)
  {
    ...
    getObjectDef()
  {
    ...
    {
            int result = AfxMessageBox("An Object that maps to a removed Object
    Description\nWould you like me to remove this object?",MB_YESNO); if (result
    == IDYES)
            {
                    Delete();
            }
            ...
            {
                    result = AfxMessageBox("Shall I point it to the next object
    description then?",MB_YESNO); if (result == IDYES) objectDefintionPtr = t;
            }
    }
    */
    return t;
  ...
  void
    setTrackSection(TrackSection *t)
  {
    trackPiece = t;
  }
    ...
    setLocator(int co)
  {
    if (co != getLocator()) {
      int result = AfxMessageBox(
        "You are about to change the object description ok?", MB_YESNO);
      if (result == IDYES) {
  ...
  void
    draw(Display *g, double x1, double y1, double angle, double trackWidth, BOOL showBitmaps, double stAngle);

  void
    draw(Display *g, double x1, double y1)
  {
  ...
  void
    write(GPTrack *track, int offset);
