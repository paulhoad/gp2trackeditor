	...
	if (pDoc)
	{
	 ...
	 if (over)
	 {
	   ...
	   if (t)
	   {
	     ...
	     TrackSection *ts = new TrackSection(over->TrackSections,over,0,20,0,0,t->getLeftToBank(),t->getRightToBank(),t->getRoadSignData(),t->getKerbData(),NULL,0,t->getWidthLeft(),t->getWidthRight(),sectionCmds,t_TrackSection);
	...
	if (pDoc)
	{
	 ...
	 if (over)
	 {
	   ...
	   if (t)
	   {
	    ...
	    //TrackCmd *trackcmd = new TrackCmd(over,0x80,0,"Inserted TrackObject");
	    TrackObject *trackcmd = new TrackObject(over,t,10,obj->getLocator(),0);
...
void TrackTool::OnLButtonUp(CView *view,CTrackEditorDoc *pDoc,UINT nFlags, CPoint point)
{
	...
	if (pDoc)
	{
	 ...
	 TrackSection *t = over->getTrackSelection();
	  if (t)
	  {
	   BOOL sel=sview->getTree()->Expand(sview->getTree()->GetParentItem(t->getTreeNode()),TVE_EXPAND);
	   TRACE("sel=%d\n"+sel);
...
void TrackTool::OnKeyDown(CView *view,CTrackEditorDoc *pDoc,UINT nChar, UINT nRepCnt, UINT nFlags)
{
	if (pDoc)
	{
	  ...
	  if (over)
	  {
		...
		case VK_9:{
				  if (t)t->setLeftToBank(t->getLeftToBank()+DELTA_WALL);over->createTrack(pDoc);
				  }break;
		case VK_0:{
				  if (t)t->setLeftToBank(t->getLeftToBank()-DELTA_WALL);over->createTrack(pDoc);
				  }break;
		case VK_O:{
				  if (t)t->setRightToBank(t->getRightToBank()+DELTA_WALL);over->createTrack(pDoc);
				  }break;
		case VK_P:{
				  if (t)t->setRightToBank(t->getRightToBank()-DELTA_WALL);over->createTrack(pDoc);
				  }break;
		...
		case VK_S:{
		   ...
		   case VK_DOWN:
				 ...
				 break;
		   case VK_DELETE:
				RemoveSection(view,pDoc);break;

		   case VK_INSERT:
				InsertSection(view,pDoc);break;
		   case 36:  ((TrackEditorScrollView *)view)->OnZoomHome();
					  
					  break;
		   case 109: ((TrackEditorScrollView *)view)->display->ZoomIn();
					  
					  break;
		   case 108: ((TrackEditorScrollView *)view)->display->ZoomOut();
...
void  ObjectTool::RemoveObject(CView *view,CTrackEditorDoc *pDoc)
{
	...
	if (pDoc)
	{
	 ...
	 if (over)
	 {
	   ...
	   if (t)
	   {
		 int result = AfxMessageBox("Removing Object From Track, Continue?",MB_YESNO);
		 if (result == IDYES)
		 {
	...
	if (pDoc)
	{
	 ...
	 if (over)
	 {
	   ...
	   if (t)
	   {
	     ...
	     TrackSection *ts = new TrackSection(over->PitlaneSections,over,0,20,0,0,t->getLeftToBank(),t->getRightToBank(),t->getRoadSignData(),t->getKerbData(),NULL,0,t->getWidthLeft(),t->getWidthRight(),sectionCmds,t_PitSection);
