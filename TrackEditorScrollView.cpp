...
#define UPDATE_TRACK(x)  CTrackEditorDoc* pDoc = GetDocument();GPTrack * mytrack = pDoc->getTrack();pCmdUI->SetCheck(mytrack->x);
...
trackImage(NULL),ruler(NULL)
{
	...
	{
	 CString file = theApp.GetProfileString("Preferences", "UnderLayImageName","none");
	 if (file != "none")
	 {
	  trackImage = new CDib();
...
BEGIN_MESSAGE_MAP(TrackEditorScrollView, CScrollView)
	...
	ON_UPDATE_COMMAND_UI(ID_SHOW_SHOWCURVEDWALLSASLINES, OnUpdateShowShowcurvedwallsaslines)
	...
	ON_UPDATE_COMMAND_UI(ID_REMOVE_REMOVEUNDERLAYIMAGE, OnUpdateRemoveRemoveunderlayimage)
	...
	ON_UPDATE_COMMAND_UI(ID_SHOW_SHOWTEXTUREMAPPINGCOMMANDS, OnUpdateShowShowtexturemappingcommands)
	ON_COMMAND(ID_SHOW_SHOWVIEWDISTANCECOMMANDS, OnShowShowviewdistancecommands)
	ON_UPDATE_COMMAND_UI(ID_SHOW_SHOWVIEWDISTANCECOMMANDS, OnUpdateShowShowviewdistancecommands)
	...
	int m_nPageHeight = pDC->GetDeviceCaps(VERTRES);
	  ...
	  if(pDC->IsPrinting())
	  {
		  if (true){			
			  ...
			  int bBandInfoDevice = pDC->Escape(QUERYESCSUPPORT, sizeof(int), 
					(LPSTR)&nTemp, NULL); 
			  ...
			  if (bBandInfoDevice)
			  {
				  ...
				  cerr << "Printer does not support margins" << endl;			      
			  }
		  }
		  else {
	...
	if(pDC->IsPrinting())
		  {
				...
				if (orientation){
				   ...
				   //pDC->SetWindowExt( rect.Width(),rect.Height()) ;   // sets   logical window units
				   //t.scale( rect.Width(), rect.Height() );
				   //t.scale( m_nPageWidth, m_nPageHeight );
				   // set the printer margin
				   
				}
				else {
				   pDC->SetWindowExt( 12*rect.Height(),12*rect.Width()) ;   // sets   logical window units
	...
	if (track)
	{
		if (display)
		{
		 ...
		 if (t && currentTool->isTrackTool())
		 {
			 ...
			 wsprintf(message,"Selected Track Section: [%d] Angle=%d Length=%d Delta_Height=%d",t->index,(int)AngleFrom(t->getAngle()),(int)t->getLength(),t->getHeight());
		 ...
		 else if (pitsel && currentTool->isPitTool())
		 {
			 ...
			 CPoint pt = GetScrollPosition();
			 pDC->SelectObject(GetStockObject(ANSI_VAR_FONT));
			 wsprintf(message,"Selected Pit Section: [%d] Angle=%d Length=%d Delta_Height=%d",pitsel->index,(int)AngleFrom(pitsel->getAngle()),(int)pitsel->getLength(),pitsel->getHeight());
			 theApp.SetMessageText(message);
...
void TrackEditorScrollView::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	CTrackEditorDoc* pDoc = GetDocument();
	if (currentTool != NULL) 
		currentTool->OnMouseMove(this,pDoc,nFlags,point);
...
void TrackEditorScrollView::OnMButtonUp(UINT nFlags, CPoint point) 
{
	CTrackEditorDoc* pDoc = GetDocument();
	if (currentTool != NULL) 
		currentTool->OnRButtonUp(this,pDoc,nFlags,point);
...
void TrackEditorScrollView::OnRButtonUp(UINT nFlags, CPoint point) 
{
	...
	CTrackEditorDoc* pDoc = GetDocument();
	if (currentTool != NULL) 
		currentTool->OnMButtonUp(this,pDoc,nFlags,point);
...
{
	// TODO: Add your command update UI handler code here
	if (currentTool != NULL) pCmdUI->Enable(currentTool->isTrackTool() || currentTool->isObjectTool());
}
...
{
	// TODO: Add your command handler code here
	AfxMessageBox("Removing Track Commands not supported from view yet used listtree!");
}
...
CTreeCtrl *TrackEditorScrollView::getTree()
{
	CTrackEditorDoc* pDoc = GetDocument();
	return pDoc->getTree();
}
...
// Insertion Macro
#define INSERT_TRACK_CMD(x) { CTrackEditorDoc* pDoc = GetDocument(); GPTrack * mytrack = pDoc->getTrack();  if (currentTool != NULL && currentTool->isTrackTool()){ TrackSection *selected = mytrack->getTrackSelection(); int selectionIndex     = mytrack->getTrackSelectionIndex();if (selected!=NULL){ selected->InsertCmdAt(selectionIndex,x);pDoc->UpdateTree(selected->getTreeNode());}else AfxMessageBox("No Track Section Selected"); }}
...
void TrackEditorScrollView::OnAddTrackside() 
{
	...
	CTextureChooser *tc = new CTextureChooser(mytrack);
	  ...
	  if (selected!=NULL)
	  {
	   ...
	   if (res!=IDCANCEL)
	   {
		 selected->AddTrackSideMarking(tc->SelectedOffset,tc->SelectedLocation,tc->SelectedLength,tc->SelectedTexture,tc->SelectedRepeats,tc->SelectedResolution,tc->SelectedYOffset,tc->SelectedRotation);
	     pDoc->UpdateTree(selected->getTreeNode());
...
#define CHECK_OUT(x)  CTrackEditorDoc* pDoc = GetDocument();GPTrack * mytrack = pDoc->getTrack();mytrack->##x = !mytrack->##x; repaint();
...
#define CHECK_GRAPH_SIZE(x)   CTrackEditorDoc* pDoc = GetDocument();GPTrack * mytrack = pDoc->getTrack();mytrack->gridSizeOption = x;
#define UPDATE_GRAPH_SIZE(x)  CTrackEditorDoc* pDoc = GetDocument();GPTrack * mytrack = pDoc->getTrack();pCmdUI->SetCheck(mytrack->gridSizeOption==x);pDoc->UpdateAllViews(NULL);
