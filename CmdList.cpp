...
CCmdList::CCmdList(CWnd* pParent /*=NULL*/) : CDialog(CCmdList::IDD, pParent)
{
...
void CCmdList::OnItemchangedList1(NMHDR* pNMHDR, LRESULT* pResult)
{
  ...
  if (pNMListView->uNewState) {
    // wsprintf(msg,"Selected %d %d %d",
    // pNMListView->iItem,pNMListView->uNewState,pNMListView->uOldState);
    // AfxMessageBox(msg);
...
BOOL CCmdList::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID, CCreateContext* pContext)
{
  ...
  m_CmdList.SetImageList(
    (((CTrackEditorApp*)AfxGetApp())->getSharedImageList()), LVSIL_SMALL);
