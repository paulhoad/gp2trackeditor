...
CTrackInfo::CTrackInfo() { trkdlg = new CTrChangeDlg(this); }
  ...
  if (mytrack != NULL) {
    ...
    if (mytrack->TrackSections) {
      for (int i = 0; i < mytrack->TrackSections->size(); i++) {
        ...
        UpdateTableStringPtr(update, i, tidx++, str, TO_ID(t->getIcon()));
        free(str);
        ...
        UPDATE_TABLE_STRING_PTR(
          update, i, tidx++, toString((int)(t->track->LengthToTrackSectorEnd(t))));
        str = toString(i);
        UpdateTableStringPtr(update, i, tidx++, str, TO_ID(t->getIcon()));
        free(str);
      }
...
void CTrackInfo::OnImportTable()
{
  ...
  if (tabledata != NULL) {
    int result = AfxMessageBox(
      "About to Import Table Data!, are you sure you want to do this",
      MB_YESNO);
