...
protected:
  ...
  // The factor by which things must be multiplied for display, == m_Numerator /
  // m_Denominator
  float m_Factor;

  // If the zoom factor has been set to a specific float value, then don't use
  // the m_Numerator:m_Denominator ratio
  BOOL m_bSpecific;
...
public:
  ...
  BOOL
    MagnifyTo(const UINT numerator);
  BOOL
    ShrinkTo(const UINT denominator);
  // Zoom or shrink to a specific floating point factor
  BOOL
    SetFactor(const float factor);
  // Area of interest zooming - return the required scroll size
  CSize
    MagnifyArea(const CSize& sizWindow, const CSize& sizAOI, const CSize& sizDoc);
  CSize
    ShrinkArea(const CSize& sizWindow, const CSize& sizAOI, const CSize& sizDoc);
  ...
  BOOL
    SetLimits(const UINT max_in, const UINT max_out);
  BOOL
    SetMagnifyLimit(const UINT max_in);
  BOOL
    SetShrinkLimit(const UINT max_out);

  // Implementation
protected:
  // Calculate the factor for multiplying, given numerator and denominator
  float
    CalcFactor(const UINT numer, const UINT denom)
  {
    return ((float)numer / (float)denom);
  }

  // Is the given zoom factor roughly equivalent to an integer ratio? e.g.
  // 1:6.001 or 1.002:1
  BOOL
    JustAboveIntegerZoom(const float factor, const float tolerance);
  BOOL
    JustBelowIntegerZoom(const float factor, const float tolerance);
};
