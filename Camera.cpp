...
CPoint
  GetCameraTrackPoint(double currrad, double angle, double length)
{
  static CPoint pt;
...
void Camera::DrawCamera(Display *g, GPTrack *track)
{
  ...
  if (track->showBitmaps) g->drawBitmap(IDB_CAMERA, bX, bY);
...
{
  track->WritePCWord(offset, X);
  offset += 2;// 0
  track->WritePCWord(offset, Y);
  offset += 2;// 2
  track->WritePCWord(offset, Z2);
  offset += 2;// 4
  track->WritePCWord(offset, UK1);
  offset += 2;// 6
  track->WritePCWord(offset, UK2);
  offset += 2;// 8
  track->WritePCWord(offset, SM);
  offset += 2;// 10
  track->WritePCWord(offset, VR1);
  offset += 2;// 12
  track->WritePCWord(offset, VR2);
  offset += 2;// 14
  track->WritePCWord(offset, ZF1);
  offset += 2;// 16
  track->WritePCWord(offset, ZF2);
  offset += 2;// 18
  track->WritePCWord(offset, UK3);
  offset += 2;// 20
  track->WritePCWord(offset, UK4);
  offset += 2;// 22
  track->WritePCWord(offset, UK5);
  offset += 2;// 24
  track->WritePCWord(offset, UK6);
  offset += 2;// 26
  track->WritePCWord(offset, UK7);
  offset += 2;// 28
  track->WritePCWord(offset, Z1);
  offset += 2;// 30
  return offset;
}
...
{
  X = track->ReadPCWord(offset);
  offset += 2;
  // int cmd       = track->ReadPCByte(offset);offset+=1;
  if (X == 0xFFFF) {
    return offset;
  }
  Y = (short)track->ReadPCWord(offset);
  offset += 2;
  Z2 = track->ReadPCWord(offset);
  offset += 2;
  UK1 = track->ReadPCWord(offset);
  offset += 2;
  UK2 = track->ReadPCWord(offset);
  offset += 2;
  SM = track->ReadPCWord(offset);
  offset += 2;
  VR1 = track->ReadPCWord(offset);
  offset += 2;
  VR2 = track->ReadPCWord(offset);
  offset += 2;
  ZF1 = track->ReadPCWord(offset);
  offset += 2;
  ZF2 = track->ReadPCWord(offset);
  offset += 2;
  UK3 = track->ReadPCWord(offset);
  offset += 2;
  UK4 = track->ReadPCWord(offset);
  offset += 2;
  UK5 = track->ReadPCWord(offset);
  offset += 2;
  UK6 = track->ReadPCWord(offset);
  offset += 2;
  UK7 = track->ReadPCWord(offset);
  offset += 2;
  Z1 = track->ReadPCWord(offset);
  offset += 2;
...
void Camera::Delete()
{
  track->MakeBackupNow();
  int res =
    AfxMessageBox("Are you sure you want to delete a Camera?", MB_YESNO);
  if (res == IDYES) {
...
double
  Camera::DistanceFromPt(CPoint pt)
{
  double dx = (pt.x - x);
  double dy = (pt.y - y);
