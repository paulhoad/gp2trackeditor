...
int CPropertySheet::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	if (!m_TrackTree.CreateEx(WS_EX_CLIENTEDGE,"SysTreeView32","",TVS_HASLINES  | TVS_HASBUTTONS | TVS_SHOWSELALWAYS | TVS_DISABLEDRAGDROP | WS_VISIBLE | WS_CHILD | WS_BORDER ,10,10,100,100,GetSafeHwnd(),(HMENU)IDC_TRACKTREE))
       return -1;
...
void CPropertySheet::OnPaint() 
{
	...
	if (m_TrackTree != NULL && ImagesNotLoaded)
		   LoadImages();
