...
LinePt* CCLineSection::drawLine(Display *g,double *x,double *y,double *curr,double nrad)
{
	if (linePt) delete linePt;
	...
	else
	{
		...
		if (getCmd()==0x50){
		  ...
		  //g->drawArc((xpos+nextxpos), (ypos+nextypos),xpos,ypos,cx,cy,getStartAngle(),getEndAngle()-getStartAngle(),rad); 
...
Vector * CCLineSection::getVector()
{
...
int RadiusVal(GPTrack *track,double rad,double direction,double arg1,int cmd,double ndirection,double arg4,double &radius2)
{
		   ...
		   int radius = (int)rad;
		   radius2 = (int)arg4;
	       if (fabs(direction) > 10.0) radius = 0.0;
...
double RadiusTo(GPTrack *track,double radius,double direction,double arg1,int cmd,double arg3,double arg4,double &rad2)
{
	       radius = (double)RadiusVal(track,radius,direction,arg1,cmd,arg3,arg4,rad2);
...
{
	track->WritePCByte(offset,(int)length);offset++;
	track->WritePCByte(offset,cmd);offset++;
	...
	if (cmd == 0x70)
	{
		track->WritePCWord(offset,(int)getArg1());offset+=2;
		track->WritePCWord(offset,scale);offset+=2;
		track->WritePCWord(offset,nRadius);offset+=2;
		track->WritePCWord(offset,(int)getArg3());offset+=2;		
		track->WritePCWord(offset,(int)getArg4());offset+=2;
	}
	else
	{
		track->WritePCWord(offset,(int)getArg1());offset+=2;
