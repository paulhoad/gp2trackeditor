...
void CWorldView::OnDraw(CDC* pDC)
{
	...
	if (showBlue) display->setColor(5);
	else display->setColor(0);

	if (isDragging())
	{
		if (WireFrameBox)
	        WireFrameBox->draw(display,cancenter);
	}
	else
	{
	  if (world)
	...
	if (mytrack->showObjects)
	{
     ...
     for(int j=0;j<TrackSideObjects->size();j++)
     {
	  ...
	  if (to!=NULL)
	  {
		 ...
		 if (t->getAngle() != 0)
		 {
		  CPoint trackPt = to->GetTrackPoint(t->getAngle(),t->getStartAngle(),to->getDistance());
          nextypos = trackPt.x;
          nextxpos = trackPt.y;
		 }
		 else
...
void CWorldView::updateBox()
{
	...
	if (WireFrameBox)
		WireFrameBox->Transform(WorldTransform,mid,center,1.0);
