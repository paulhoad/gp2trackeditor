...
void GPTrack::drawTrackLines(Display *g)
{
	   ...
	   for(int i=0;i<size;i++)
       {
		   ...
		   if (showTrack)
             t->drawTrackLine(g,t->getAngle(),this);
...
LinePt *TrackSection::drawTrackLine(Display *g,double angle,GPTrack *track)
{
...
double distanceTo(LinePt *A,CPoint B)
{
	double dx = A->x - B.x;
	double dy = A->y - B.y;
...
Vector * GPTrack::getSelectedSections(CPoint begin,CPoint end)
{
...
char * GPTrack::GetJAMFile(int id)
{
	int size = JamFiles->size();
	int count=0;
...
Camera * GPTrack::DistanceToCamera(double distance)
{
	Camera *verLast = (Camera*)TrackCameras->elementAt((TrackCameras->size()-1));
	...
	for(int i=0;i<TrackCameras->size();i++)
	{
	   ...
	   if (distance >= lastSwitchMoment && distance <sm)
		   return cam;
	}
