...
CObjectView::CObjectView() : inter_obj(NULL) {}
...
void CObjectView::UpdateInfo(BOOL update)
{
  ...
  if (mytrack != NULL) {
    ...
    if (obj) {
      ...
      CString OffsetDesc[] = { "Pointer to Scale Section",
      ...
      for (int i = pos; i < obj->objectData->size(); i++) {
        dat = (ObjectData *)obj->objectData->elementAt(i);
  ...
  if (obj) {
    ...
    int LastDataSize =
      ((ObjectData *)obj->objectData->elementAt(24))->GetValue();
    ...
    UpdateTableStringPtr(
      FALSE, idx++, DESC_COL, CString("val - val1 = Size of 2nd sector after vertex list"));
    UpdateTableStringPtr(
      FALSE, idx++, DESC_COL, CString("val - val1 = Size of 3nd sector after vertex list"));
    // 78  scale
