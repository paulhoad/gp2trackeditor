...
class BBox3D
{
	...
	void adjustWith(double x,double y,double z)
	{
		xmin = min(xmin,x);
		ymin = min(ymin,y);
		zmin = min(zmin,z);
...
public:
	...
	virtual void draw(Display *d,Point3D *center)
	{}

	virtual void fill(Display *d,Point3D *center)
	{}

	virtual void Transform(Matrix3D *mat, Point3D *mid,Point3D *center, double scalefactor)
	{
	}

	virtual BBox3D *findBoundingBox()
	{
		return NULL;
	}
};
...
class Matrix3D 
{
	...
	Point3D *transform(Point3D *v, Point3D *mid);
...
public:
	...
	void draw(Display *d,Point3D *center)
	{
	   d->drawLine(center->x+SCALE(tx)-2,center->y+SCALE(ty)-2,center->x+SCALE(tx)+2,center->y+SCALE(ty)+2);
	   d->drawLine(center->x+SCALE(tx)-2,center->y+SCALE(ty)+2,center->x+SCALE(tx)+2,center->y+SCALE(ty)-2);
	}

	void fill(Display *d,Point3D *center)
	{
	   d->drawLine(center->x+SCALE(tx)-2,center->y+SCALE(ty)-2,center->x+SCALE(tx)+2,center->y+SCALE(ty)+2);
	   d->drawLine(center->x+SCALE(tx)-2,center->y+SCALE(ty)+2,center->x+SCALE(tx)+2,center->y+SCALE(ty)-2);
	}

	void Transform(Matrix3D *mat, Point3D *mid,Point3D *center, double scalefactor)
	{
...
public:
	...
	void addPrimitive(Primitive3D *prim)
    {
	...
	Model3D *findPointsBox()
		{
		   Model3D *box = new Model3D();

		   BBox3D *worldExtent = findBoundingBox();

		   Point3D *A = new Point3D(worldExtent->xmin,worldExtent->ymin,worldExtent->zmin);
		   Point3D *B= new Point3D(worldExtent->xmax,worldExtent->ymin,worldExtent->zmin);
		   Point3D *C= new Point3D(worldExtent->xmax,worldExtent->ymax,worldExtent->zmin);
		   Point3D *D= new Point3D(worldExtent->xmin,worldExtent->ymax,worldExtent->zmin);

		   Point3D *E= new Point3D(worldExtent->xmin,worldExtent->ymin,worldExtent->zmax);
		   Point3D *F= new Point3D(worldExtent->xmax,worldExtent->ymin,worldExtent->zmax);
		   Point3D *G= new Point3D(worldExtent->xmax,worldExtent->ymax,worldExtent->zmax);
		   Point3D *H= new Point3D(worldExtent->xmin,worldExtent->ymax,worldExtent->zmax);
...
public:
		...
		WorldView3D()
	    {
			models = new Vector("WorldView3D",__LINE__);
	    }
