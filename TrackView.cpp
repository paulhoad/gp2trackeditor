...
void CTrackView::OnDraw(CDC* pDC)
{
	...
	// Show non-picture areas as grey hatch - could use solid COLOR_APPWORKSPACE instead, I suppose
	CBrush brBorder;
	...
	if (sizDraw.cx > sizZoomedTrack.cx)
		sizDraw.cx = sizZoomedTrack.cx;
	if (sizDraw.cy > sizZoomedTrack.cy)
		sizDraw.cy = sizZoomedTrack.cy;
	...
	if (sizBmpView.cx > sizTrack.cx)
		sizBmpView.cx = sizTrack.cx;
	if (sizBmpView.cy > sizTrack.cy)
		sizBmpView.cy = sizTrack.cy;

	// Copy the bitmap
	pDC->StretchBlt(ptScrollOffset.x, ptScrollOffset.y, sizDraw.cx, sizDraw.cy,
		&copyDC, ptBmpOffset.x, ptBmpOffset.y, sizBmpView.cx, sizBmpView.cy,
		SRCCOPY);
...
CTrackEditorDoc* CTrackView::GetDocument() // non-debug version is inline
{
...
void CTrackView::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	if (GetCurrentTool() == ID_TOOL_ZOOM)
	{
		...
		if (m_bDragging)
		{
			...
			CSize sizScroll = m_pZoom->MagnifyArea(rctClient.Size(), rctRubberBand.Size(), sizTrack);

			// Delay redrawing the view
			InvalidateRect(NULL, FALSE);

			// Set the scroll size
			if (sizScroll != CSize(-1,-1))
...
void CTrackView::OnRButtonUp(UINT nFlags, CPoint point) 
{
	...
	if (GetCurrentTool() == ID_TOOL_ZOOM)
	{
		...
		if (m_bDragging)
		{
			...
			// Set the new zoom factor
			CSize sizScroll = m_pZoom->ShrinkArea(rctClient.Size(), rctRubberBand.Size(), sizTrack);
...
void CTrackView::ClientToDoc(CPoint& ptConv, CPoint scroll_pos /* = CPoint(-1,-1)*/,
							 float zoom_factor /*= 0.0f*/)
{
	// If scroll position and/or zoom factor have not been supplied, use current values
	CPoint ptScrollPos = (CPoint(-1,-1) != scroll_pos) ? scroll_pos : GetScrollPosition();
	float fZoomFactor = (0.0f != zoom_factor) ? zoom_factor : m_pZoom->GetFactor();
...
void CTrackView::DocToClient (CPoint& ptConv, CPoint scroll_pos /* = CPoint(-1,-1)*/,
							 float zoom_factor /*= 0.0f*/)
{
	// If scroll position and/or zoom factor have not been supplied, use current values
	CPoint ptScrollPos = (CPoint(-1,-1) != scroll_pos) ? scroll_pos : GetScrollPosition();
...
{
	// Make sure the caller is passing in a proper parameter for the zoom direction (not sure
	// how strong type-checking is for enumerated types)
	...
	for (UINT nStepNum = 0; nStepNum < nStepCount; nStepNum++)
	{
		...
		if (ZOOM_IMAGE_IN == zoom_dirn)
		{
			...
			rctDraw.bottom = rctZoom.bottom + (nStepNum * (rctDiff.bottom / nStepCount));
		}
		else
		{
			rctDraw.left = rctClient.left + (nStepNum * (rctDiff.left / nStepCount));
			rctDraw.top = rctClient.top + (nStepNum * (rctDiff.top / nStepCount));
			rctDraw.right = rctClient.right - (nStepNum * (rctDiff.right / nStepCount));
			rctDraw.bottom = rctClient.bottom - (nStepNum * (rctDiff.bottom / nStepCount));
		}
...
// The point passed in is in document coordinates, not logical or device coordinates
BOOL CTrackView::ZoomToPoint (const CPoint& ptZoomCenter, const eZoomDirection zoom_dirn)
{
	// Make sure the caller is passing in a proper parameter for the zoom direction (not sure
	// how strong type-checking is for enumerated types)
