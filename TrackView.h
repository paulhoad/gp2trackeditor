...
#define ZOOM_BOUNCE		4	// Number of pixels movement before
...
protected:
	...
	void CancelDrag(BOOL bKillRubberBand);
	void MoveRubberBand(CPoint ptNewEndPoint);
	void DrawRubberBand (CPoint ptStart, CPoint ptEnd);
	BOOL AnimateZoom (const CRect& rctZoom, const eZoomDirection zoom_dirn);

	// Coordinate conversion functions
	void ClientToDoc(CPoint& ptConv, CPoint scroll_pos = CPoint(-1,-1), float zoom_factor = 0.0f);
	void DocToClient(CPoint& ptConv, CPoint scroll_pos = CPoint(-1,-1), float zoom_factor = 0.0f);

	// Main single-click zoom function
	BOOL ZoomToPoint (const CPoint& ptZoomCenter, const eZoomDirection zoom_dirn);
...
inline CTrackEditorDoc* CTrackView::GetDocument()
