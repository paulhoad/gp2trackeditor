...
BOOL CObjectEdit::OnInitDialog() 
{
	...
	if (m_Detail==4)   
		m_DetailCombo.SetCurSel(0);

	
	if (m_Detail==100) 
		m_DetailCombo.SetCurSel(1);
	
	if (m_Detail==200) 
		m_DetailCombo.SetCurSel(2);

	if (m_Detail==64) 
		m_DetailCombo.SetCurSel(2);

	if (m_Detail==148) 
		m_DetailCombo.SetCurSel(3);
...
void CObjectEdit::OnSelchangeDetailcombo() 
{
	...
	switch(idx)
	{
	   case 0: m_Detail = 4 ;break;
	   case 1: m_Detail = 100 ;break;
	   case 2: m_Detail = 64 ;break;
	   case 3: m_Detail = 2 ;break;
	   case 4: m_Detail = 148 ;break;
	}
...
void CObjectEdit::OnButton2() 
{
	...
	if (m_Id1 > 17)
	{
	 ...
	 {
	  InternalObject *t = (InternalObject*)track->InternalObjectOffsets->elementAt(internalObjectId);
...
void CObjectEdit::OnButton3() 
{
	...
	if (m_Id1 > 17)
	{
	 ...
	 if (internalObjectId >= 0 && internalObjectId < numberOfInternalObjects)
	 {
	  InternalObject *t = (InternalObject*)track->InternalObjectOffsets->elementAt(internalObjectId);
	  t->Expand(theApp.getMainTree(),t->getTreeNode());
	  theApp.getMainTree()->getTree()->Expand(theApp.getMainTree()->getTree()->GetParentItem(t->getTreeNode()),TVE_EXPAND);
