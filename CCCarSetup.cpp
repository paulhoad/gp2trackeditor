...
int CCCarsetup::Open()
{
		...
		if (dlg->m_Accel > 128)
		{
			AfxMessageBox("Accel == -ve car may go backwards",MB_OK | MB_ICONWARNING);
		}
		CCAccel = dlg->m_Accel;
...
int  CCCarsetup::read(GPTrack *track,int offset)
{
		...
		CCFrontWing  = track->ReadPCByte(offset)-0x97;offset+=1;
		CCRearWing   = track->ReadPCByte(offset)-0x97;offset+=1;
		CCSetpData[0].SetDesc("Front Wing + 0x97/ Rear Wing + 0x97");

		CCFirstGear  = track->ReadPCByte(offset)-0x97;offset+=1;
		CCSecondGear = track->ReadPCByte(offset)-0x97;offset+=1;
		CCSetpData[1].SetDesc("1st Gear + 0x97/ 2nd Gear + 0x97");

		CCThirdGear  = track->ReadPCByte(offset)-0x97;offset+=1;
		CCFourthGear = track->ReadPCByte(offset)-0x97;offset+=1;
		CCSetpData[2].SetDesc("3rd Gear + 0x97/ 4th Gear + 0x97");

		CCFifthGear  = track->ReadPCByte(offset)-0x97;offset+=1;
		CCSixthGear  = track->ReadPCByte(offset)-0x97;offset+=1;
		CCSetpData[3].SetDesc("5th Gear + 0x97/ 6th Gear + 0x97");

		CCTyreType   = track->ReadPCByte(offset);offset+=1;
...
int CCCarsetup::write(GPTrack *track,int offset)
	{
		...
		track->WritePCByte(offset,(CCFrontWing+0x97));offset+=1;
		track->WritePCByte(offset,(CCRearWing+0x97));offset+=1;
		track->WritePCByte(offset,(CCFirstGear+0x97));offset+=1;
		track->WritePCByte(offset,(CCSecondGear+0x97));offset+=1;
		track->WritePCByte(offset,(CCThirdGear+0x97));offset+=1;
		track->WritePCByte(offset,(CCFourthGear+0x97));offset+=1;
		track->WritePCByte(offset,(CCFifthGear+0x97));offset+=1;
		track->WritePCByte(offset,(CCSixthGear+0x97));offset+=1;
		track->WritePCByte(offset,(CCTyreType));offset+=1;
