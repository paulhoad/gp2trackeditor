...
void LoadToolBarInfo(CWinApp* app,CString & registryEntry, DWORD * style, CRect & toolRect, bool * docked, UINT * dockBarID)
{
	...
	*docked = (app->GetProfileInt(registryEntry, "Docked", 1)) ? true : false ;
	if (!(*docked))	{
		...
		(*toolRect).left	= app->GetProfileInt(registryEntry, "left", (*toolRect).left);
		...
		*dockBarID			= 0;
		...
		(*toolRect).top		= app->GetProfileInt(registryEntry, "top", (*toolRect).top);
		(*toolRect).left	= app->GetProfileInt(registryEntry, "left", (*toolRect).left);
		(*toolRect).bottom	= app->GetProfileInt(registryEntry, "bottom", (*toolRect).bottom);
		(*toolRect).right	= app->GetProfileInt(registryEntry, "right", (*toolRect).right);
...
void LoadToolBar(CWinApp* app, CFrameWnd * frame, CToolBar * toolBar,CString registryEntry, 
						DWORD style, UINT dockingBarID)
{
	bool docked = true;
	CRect rect(0,0,0,0);
	toolBar->GetWindowRect( &rect );
	LoadToolBarInfo(app, registryEntry, &style, rect, &docked, &dockingBarID);
	if (docked)		frame->DockControlBar(toolBar, dockingBarID, rect);
	else			frame->FloatControlBar(toolBar, CPoint( rect.left, rect.top ), style );
...
void LoadDockBar(CWinApp* app, CFrameWnd * frame, CString & registryEntry, UINT bar)
{
	...
	{	
		...
		if ((rect.top != rect.bottom) && ( rect.top != -1))
			pDock->SetWindowPos(NULL, rect.left, rect.top, rect.Width(),rect.Height(), 
