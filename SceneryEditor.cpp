...
void CSceneryEditor::DrawScenery(CDC *pDC, RECT &rect)
{
  ...
  if (currentSection && currentSection->containsCommand(0xb8)) {
    ...
    if (currentCmd) {
      ...
      if (detX != 0) scaleX = (right - left) / (detX);

      if (detZ != 0) scaleZ = (top - bottom) / (detZ);
      ...
      for (int i = 0; i < 6; i++) {
        ...
        // BOOL drawIt = (m_ShowHidden ||
        // track->isRibbonOn(currentSection->getIndex(),IntToRibbon2(i)));
...
void CSceneryEditor::OnSize(UINT nType, int cx, int cy)
{
  ...
  if (wnd != NULL) {
    ...
    wnd = GetDlgItem(IDC_HEIGHTDRAW);
    if (wnd)
      wnd->SetWindowPos(NULL, 10, midy + 10, width, height2, SWP_NOZORDER);

    wnd = GetDlgItem(IDC_PROFILE);
    if (wnd)
      wnd->SetWindowPos(NULL, 15, midy - 5, 0, 0, SWP_NOSIZE | SWP_NOZORDER);

    wnd = GetDlgItem(IDOK);
    if (wnd)
      wnd->SetWindowPos(NULL, 15, cy - 30, 0, 0, SWP_NOSIZE | SWP_NOZORDER);
    wnd = GetDlgItem(IDCANCEL);
    if (wnd)
      wnd->SetWindowPos(NULL, 15 + 80 + 20, cy - 30, 0, 0, SWP_NOSIZE | SWP_NOZORDER);
    wnd = GetDlgItem(IDC_PREV);
    if (wnd)
      wnd->SetWindowPos(NULL, cx - 15 - 80 - 80 - 20, cy - 30, 0, 0, SWP_NOSIZE | SWP_NOZORDER);
    wnd = GetDlgItem(IDC_NEXT);
    if (wnd)
      wnd->SetWindowPos(NULL, cx - 15 - 80, cy - 30, 0, 0, SWP_NOSIZE | SWP_NOZORDER);
  }
}
...
void CSceneryEditor::DrawProfile(CDC *pDC, RECT &rect)
{
  ...
  if (pitlane) total = track->totalPitLength();
...
LRESULT
CSceneryEditor::DefWindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
  // TODO: Add your specialized code here and/or call the base class
...
#define UPDATE_SCX(a)                                            \
  if (currentCmd != NULL)                                        \
    currentCmd->setNthArg((2 * a) + 1, GetDlgItemInt(IDC_X##a)); \
  edited = TRUE;                                                 \
  GetDlgItem(IDC_DRAW)->InvalidateRect(NULL);
#define UPDATE_SCZ(a)                                            \
  if (currentCmd != NULL)                                        \
    currentCmd->setNthArg((2 * a) + 2, GetDlgItemInt(IDC_Z##a)); \
  edited = TRUE;                                                 \
  GetDlgItem(IDC_DRAW)->InvalidateRect(NULL);
