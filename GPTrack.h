...
public:
	...
	void WriteInfo();
	void ReadInfo(BOOLEAN complainNoGPInfo=TRUE);
	void ReadTrackFile(CDocument *pDoc,const char * filename);
	int ReadTrackInfoFile(const char * filename);
	void WriteTrackFile(const char * filename);
	void ParseTrack(CDocument *pDoc);
	void ReadOffsets(CDocument *pDoc);
	...
	char * ReadNullTerminatedString(int start);
	void WriteNullTermintedString(int start,char *file,int len);
	void createTrack(CDocument *pDoc);
	void createPitlane();
	void createCC();
	void drawTrack(Display *g,BOOL drawGray=FALSE);
	void drawPitlane(Display *g);
	void drawCCLine(Display *g);
	void drawCameras(Display *g);
	void drawBlackFlags(Display *g);
	void drawPitCmds(Display *g);
	void drawTrackMarkings(Display *g);
	void drawVarious(Display *g);
	void CreateVRMLFile(char *filename);

	Vector *getSelectedSections(CPoint begin,CPoint end);

	BOOL regenerate;

	double  LengthToTrackSector(TrackSection *in);
    TrackSection *SectorForLength(double len);
	double  LengthToPitSectorEnd(TrackSection *in);
	...
	void ScaleTrack(float scale);

	int  ReadPCDWord(int offset);
	int  ReadPCByte(int offset);
	int  ReadPCWord(int offset);
	void WritePCWord(int offset,int value);
	void WritePCDWord(int offset,int value);
	void WritePCByte(int offset,int value);
	...
	void GetTrackObjectDefinition(int start);
	...
	void ReadTrackObjectDefinitions(int start,int end);
	TrackObjectDefinition *getObjectDefinition(int code);

	TrackObjectDefinition * FindObjectDefinition(int Offset);
	TrackObjectDefinition * AddNewTrackObjectDefinition();
	TrackSection * GetTrackSection(Vector *vec,int idx);

	void drawGrid(Display *g);

	Model3D  *createSceneryModel(Display *g);
	...
	void setBestSlotInfo(int slot)
	{
		circuitSlot = slot;
	}

	void setTyreConsumption(int tyre)
	{
		circuitTyreConsumption = tyre;
	}

	CString getLapRecord()
	{
		return lapRecord;
	}

	CString getLapRecordQualify()
	{
		return lapRecordQualify;
	}

	void setLapRecordQualify(CString str)
	{
		lapRecordQualify = CString(str);
	}

	void setLapRecord(CString str)
	{
		lapRecord = CString(str);
	}

	CString getCircuitName()
	{
		return circuitName;
	}

	CString getCircuitCopyright()
	{
		return circuitCopyright;
	}

	CString getCircuitCountry()
	{
		return countryName;
	}

	CString getCircuitYears()
	{
		return circuitYear;
	}

	CString getCircuitAuthor()
	{
		return authorName;
	}

	CString getCircuitEvent()
	{
		return circuitEvent;
	}

	CString getCircuitDesc()
	{
		return circuitDesc;
	}
	...
	void  setCircuitName(CString name)
	{
		//if (circuitName) delete circuitName;
		circuitName = CString(name);
	}
	void  setCircuitCopyright(CString name)
	{
		//if (circuitCopyright) delete circuitCopyright;
		circuitCopyright = CString(name);
	}
	void  setCircuitCountry(CString name)
	{
		//if (countryName) delete countryName;
		countryName =  CString(name);
	}
	void  setCircuitYears(CString name)
	{
		//if (circuitYear) delete circuitYear;
		circuitYear = CString(name);

	}
	void  setCircuitAuthor(CString name)
	{
		//if (authorName) delete authorName;
		authorName =  CString(name);

	}
	void  setCircuitEvent(CString name)
	{
        //if (circuitEvent) delete circuitEvent;
		circuitEvent =  CString(name);
	}
	void setCircuitDesc(CString name)
	{
		//if (circuitDesc) delete circuitDesc;
		circuitDesc =  CString(name);
	}
    void setCircuitReal(BOOL real)
	{
		 circuitReal = real;
	}
	void setCircuitLaps(int laps)
	{
		 circuitLaps = laps;
	}


    TrackSection * GetTrackSection(int idx);	
    int GetTrackSectionIndex(TrackSection *t);
	TrackSection * GetNextSection(TrackSection *t);
    TrackSection * GetPreviousSection(TrackSection *t);
	

	CCLineSection * GetCCSection(int idx);
	int GetCCSectionIndex(CCLineSection *t);
    CCLineSection * GetNextSection(CCLineSection *t);
    CCLineSection * GetPreviousSection(CCLineSection *t);
	Camera        *GetPreviousCamera(Camera *t);
	TrackSection * FindTrackCommand(int cmd,int pos=0);


	BOOL PitLaneEntranceToLeft();
	BOOL PitLaneExitToLeft();

	void drawTrackLines(Display *g);
	void drawPitLines(Display *g);
	void drawCCLines(Display *g);
	...
	BOOL isCircuitDefined();
	BOOL isCountryDefined();
	...
	TrackObject* GetTrackObject(int offset,int len,Vector *sectorCmds);
	...
	Camera *GetCamera(int offset,int num);
	//int PutTrackSection(TrackSection *t);
	//int PutPitLaneSection(TrackSection *t);
	int PutCCSection(CCLineSection *t);

	void selectedSection(CPoint pt);
	void selectedPitSection(CPoint pt);
	void selectedCCSection(CPoint pt);
	TrackSection *getTrackSelection();
	TrackSection *getPitLaneSelection();
	CCLineSection *getCCSelection();
	...
	void clearTrackSelections();
	void clearPitSelections();
	void setTrackSelection(TrackSection *t);
	void setPitSelection(TrackSection *t);
	void setCCSelection(CCLineSection *t);

	TrackObject* selectedObject(CPoint pt);
	TrackObject* GetSelectedObject();

	Camera* selectedCamera(CPoint pt);
	Camera* GetSelectedCamera();
	void    clearSelectedCameras();
	void    setCameraSelection(Camera *cam);

	void DrawSplits(Display *display);
	...
	BOOL isRibbonOn(int ts,int myribbon);
	...
	void RemoveAllCmds(int cmdval,int startingAt=0,int finishinAt=-1);
	void ZeroAllArgs(int cmdval);
	void DefaultAllArgs(int cmdval);
	void RemoveAllObjects();
	void RemoveAllPitCmds();
	void RemoveAllPitObjects();

	void clearCCSelection();

	void RemoveCommand(TrackCmd *cmd);
	...
	double LengthToTrackSectorEnd(TrackSection *in);

	JAM *GetJAM(int id);
	char * GetJAMFile(int id);
	

	void WriteSum(int offset,BOOL write,BOOL dialog)
	{
		char mess[256];
	...
	{         
	   ...
	   d1 = d1 & 0x000000FF;  // only look at the bottom 8 bytes of d1 so d1 will be 0-255;
	...
	void GuessTrackIdenty(BOOL complain=TRUE);

	
	CCCarsetup *carsetup;

	void ExportTrackCommands(LPCSTR filename);

	void ImportNewInternalObject(CDocument *pDoc,LPCSTR filename);

	void ExportCleverNames();
	...
	void drawPitToTrackConnections(Display *g);

	void ImportCCDataLog(LPCSTR filename);
	void ImportTrackSegmentsDataLog(LPCSTR filename);
    void ExportXYDataLog(LPCSTR filename);

	void DrawCCDataLog(Display *g);
	Vector *CCDataLog;
	...
	void Insert112CmdSection();

	Camera * DistanceToCamera(double distance);

	void DrawObjectOutline(Display *g,TrackSection *t);
...
double RadiusTo(GPTrack *track,double radius,double direction,double arg1,int cmd,double arg3,double arg4,double &radius2);
