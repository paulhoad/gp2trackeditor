...
BOOL CDib::Read(CFile* pFile) //CFile ptr passed by the documents serialize function
{
	...
	TRY {
		...
		//check if compressed
		if (!((m_lpBMIH->biCompression==BI_RGB) || (m_lpBMIH->biCompression==BI_RLE8)
...
{	//rotate DIB by theta radians. If enlarge is TRUE the size of the rotated
	...
	for (int x=newrect.left; x<=newrect.right; x++) {
		for (int y=newrect.top; y<=newrect.bottom; y++) {
			...
			if ((pntFrom.x>=0) && (pntFrom.x<dim.cx) &&
				(pntFrom.y>=0) && (pntFrom.y<dim.cy)) {
				double a = fpntFrom.x-floor(fpntFrom.x);
				double b = fpntFrom.y-floor(fpntFrom.y);
				if ((pntFrom.x==(dim.cx-1)) || (pntFrom.y==(dim.cy-1))) {
					newpix = (int)((1.0-a)*(1.0-b)*(double)(pImage[pntFrom.y][pntFrom.x])
						+ (1.0-a)*b*GetBorderPixel(pImage,pntFrom.x,pntFrom.y+1)
						+ a*(1.0-b)*GetBorderPixel(pImage,pntFrom.x+1,pntFrom.y)
						+ a*b*GetBorderPixel(pImage,pntFrom.x+1,pntFrom.y+1));
				}
				else {
					newpix = (int)((1.0-a)*(1.0-b)*(double)(pImage[pntFrom.y][pntFrom.x])
						+ (1.0-a)*b*(double)(pImage[pntFrom.y+1][pntFrom.x])
						+ a*(1.0-b)*(double)(pImage[pntFrom.y][pntFrom.x+1])
						+ a*b*(double)(pImage[pntFrom.y+1][pntFrom.x+1]));
				}
...
void CDib::Scale(double factorX, double factorY)
{
	...
	for(nYValue=0;nYValue<(nNewHeight-(int)ceil(factorY));nYValue++){ // -factorY = do not calculate lower left corner
		...
		for(nXValue=(nNewWidth-(int)ceil(factorX));nXValue<(nNewWidth);nXValue++){ // -factorX = do not calculate at border
			...
			lpNewImage[YIndexInNewIm + nXValue] =
				(BYTE)((1.0-a)*(1.0-b)*(double)ImageMatrix[wFromY][wFromX]
				+ (1.0-a)*b*(double)ImageMatrix[wFromY+1][wFromX]
				+ a*(1.0-b)*(double)ImageMatrix[wFromY][wFromX] // indexing out of image - uses pntFrom-value
				+ a*b*(double)ImageMatrix[wFromY][wFromX]); // indexing out of image - uses pntFrom-value
		}
	}
	...
	for(nYValue=(nNewHeight-(int)ceil(factorY));nYValue<nNewHeight;nYValue++){
		...
		for(nXValue=0;nXValue<(nNewWidth-(int)ceil(factorX));nXValue++){ // -factorX = do not calculate lower left corner
			...
			lpNewImage[YIndexInNewIm + nXValue] =
				(BYTE)((1.0-a)*(1.0-b)*(double)ImageMatrix[wFromY][wFromX]
				+ (1.0-a)*b*(double)ImageMatrix[wFromY][wFromX] // indexing out of image - uses pntFrom-value
				+ a*(1.0-b)*(double)ImageMatrix[wFromY][wFromX+1]
				+ a*b*(double)ImageMatrix[wFromY][wFromX]); // indexing out of image - uses pntFrom-value
		}
	}
...
BOOL CDib::Blur(BYTE byBlurFactor){
	...
	for(WORD wXCount=0; wXCount<m_lpBMIH->biWidth;wXCount++){
		for(WORD wYCount=0; wYCount<m_lpBMIH->biHeight;wYCount++){
			wSumPixels=0;
			wNrOfPixels=0;
			for(nLocalXC=(wXCount-byBlurFactor); nLocalXC<=(wXCount+byBlurFactor);nLocalXC++){
				for(nLocalYC=(wYCount-byBlurFactor); nLocalYC<=(wYCount+byBlurFactor);nLocalYC++){
...
{	//Smoothes the histogram to remove local variations by changing each value
	...
	int color, i, count, hw=(width-1)/2;
	DWORD sum;

	for (color=0; color<=m_nColorTableEntries-1; color++){
		sum = 0;
		count = 0;
		for (i=color-hw; i<=color+hw; i++){
...
CFloatPoint CDib::FromPoint(CPoint pnt, double theta)
{	//finds point to get pixel in target-to-source rotation

	CFloatPoint fPnt;

	fPnt.x = (pnt.x-m_fpntRotOrigo.x)*cos(theta)-(pnt.y-m_fpntRotOrigo.y)*sin(theta)
		   + m_fpntRotOrigo.x;
	fPnt.y = (pnt.x-m_fpntRotOrigo.x)*sin(theta)+(pnt.y-m_fpntRotOrigo.y)*cos(theta)
		   + m_fpntRotOrigo.y;
	return fPnt;
}

CFloatPoint CDib::ToPoint(CPoint pnt, double theta)
{	//finds where point maps to in source-to-target rotation
	...
	fPnt.x = (pnt.x-m_fpntRotOrigo.x)*cos(theta)+(pnt.y-m_fpntRotOrigo.y)*sin(theta)
		   + m_fpntRotOrigo.x;
	fPnt.y = (m_fpntRotOrigo.x-pnt.x)*sin(theta)+(pnt.y-m_fpntRotOrigo.y)*cos(theta)
		   + m_fpntRotOrigo.y;
	return fPnt;
}
...
CRect CDib::CalcNewCanvas(const CSize& dim, double theta, BOOL enlarge)
{
	if (enlarge) {
		...
		CPoint pntMin(round(min(0,min(min(ul.x,ur.x),min(lr.x,ll.x)))),
					  round(min(0,min(min(ul.y,ur.y),min(lr.y,ll.y)))));
		CPoint pntMax(round(max(dim.cx-1,max(max(ul.x,ur.x),max(lr.x,ll.x)))),
