...
class JAMEntry : public CObject
{
   ...
   void LoadTree(CTrackTree *tree,HTREEITEM root)
	{
		CString buffer;
		...
		ptr = palette2;
		for( i=0;i<info->palette_size_div4;i++)
		{
		  buffer.Format("Val:%d",(int)*ptr);
		  ptr++;
...
class JAM : public CObject
{
   public:
	...
	HTREEITEM LoadTree(CTrackTree *tree,HTREEITEM root,BOOL loaded)
	{
		CString buffer;
	...
	int GetFileLength()
	{
		...
		if (fpJam){
            result = _fstat( fileno(fpJam), &buf );
		}
		else {
			return 0;
		}
