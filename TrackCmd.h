...
class TrackCmd : public Observer
  , public CObject
{
public:
  int cmd;
  ...
  TrackCmd(GPTrack *mytrack, TrackSection *t, int cmd, int offset, LPCSTR name, int arg1 = -1, int arg2 = -1, int arg3 = -1, int arg4 = -1, int arg5 = -1, int arg6 = -1, int arg7 = -1, int arg8 = -1, int arg9 = -1, int arg10 = -1)
    : cmd(cmd),
      offset(offset),
      name(name),
      Observer(mytrack, t_Command, (Observer *)t),
      owner(t)
  {
    ...
    if (arg2 != -1) {
      TrackArg *arg = new TrackArg(arg2, GP_CMD_WORD, "Unk");
      args->addElement(arg);
    }
    if (arg3 != -1) {
      TrackArg *arg = new TrackArg(arg3, GP_CMD_WORD, "Unk");
      args->addElement(arg);
    }
    if (arg4 != -1) {
      TrackArg *arg = new TrackArg(arg4, GP_CMD_WORD, "Unk");
      args->addElement(arg);
    }
    if (arg5 != -1) {
      TrackArg *arg = new TrackArg(arg5, GP_CMD_WORD, "Unk");
      args->addElement(arg);
    }
    if (arg6 != -1) {
      TrackArg *arg = new TrackArg(arg6, GP_CMD_WORD, "Unk");
      args->addElement(arg);
    }
    if (arg7 != -1) {
      TrackArg *arg = new TrackArg(arg7, GP_CMD_WORD, "Unk");
      args->addElement(arg);
    }
    if (arg8 != -1) {
      TrackArg *arg = new TrackArg(arg8, GP_CMD_WORD, "Unk");
      args->addElement(arg);
    }
    if (arg9 != -1) {
      TrackArg *arg = new TrackArg(arg9, GP_CMD_WORD, "Unk");
      args->addElement(arg);
    }
    if (arg10 != -1) {
      TrackArg *arg = new TrackArg(arg10, GP_CMD_WORD, "Unk");
      args->addElement(arg);
    }
  }
  ...
  int write(GPTrack *track, int offset)
  {
    ...
    track->WritePCByte(offset, TrackArg1);
    offset += 1;
    ...
    for (int i = 1; i < numArgs; i++) {
      ...
      switch (type) {
      case GP_CMD_BYTE:
        track->WritePCByte(offset, value);
        offset += 1;
        break;
      case GP_CMD_WORD:
        track->WritePCWord(offset, value);
        offset += 2;
        break;
      case GP_CMD_DWORD:
        track->WritePCDWord(offset, value);
        offset += 4;
        break;
      }
    }
  ...
  int TrackCmd::debug(FILE *fp, GPTrack *track, int offset)
  {
    ...
    fprintf(fp, "0x%d Cmd:%d %s\n", offset, cmd, (LPCSTR)getCmdDescription());
    fprintf(fp, "\tArg(1)=%d\n", TrackArg1);
    ...
    for (int i = 1; i < args->size(); i++) {
      ...
      fprintf(fp, "\tArg(%d)=%d %s %s\n", i + 1, value, ArgType[type], (LPCSTR)getCmdNthArgDescription(i + 1));
  ...
  CString
    getCmdDescription();
  CString
    getCmdNthArgDescription(int arg);
  ...
  void
    AddWordArgument(int arg, LPCSTR name)
  {
  ...
  DataValue *
    getNthArgDataValue(int idx)
  {
    TrackArg *targ = (TrackArg *)args->elementAt(idx - 1);
    if (targ) return &(targ->value);
    return NULL;
  }

  virtual void
    setNthArg(int idx, int val)
  {
    TrackArg *targ = (TrackArg *)args->elementAt(idx - 1);
    if (targ)
      targ->setValue(val);
