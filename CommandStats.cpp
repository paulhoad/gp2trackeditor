...
extern int
  getDisplayIconType(GPTrack *track, int locator, int id1, int id2);
  ...
  if (mytrack != NULL) {
    ...
    for (int j = 0; j < 255; j++) {
      ...
      for (int i = 0; i < mytrack->PitlaneSections->size(); i++) {
        TrackSection *t =
          (TrackSection *)mytrack->PitlaneSections->elementAt(i);
        Vector *cmds = t->getCommands();
    ...
    for (int i = 0; i < mytrack->TrackObjectDefintions->size(); i++) {
      TrackObjectDefinition *def =
        (TrackObjectDefinition *)mytrack->TrackObjectDefintions->elementAt(i);
      char buffer[256];
      ...
      tidx = 0;
      UpdateTableStringPtr(
        update, rowidx, tidx++, toHexString(i), TO_ID(getDisplayIconType(mytrack, def->locator, def->id, def->id2)));
      UpdateTableStringPtr(update, rowidx, tidx++, toString(i));
      UpdateTableStringPtr(update, rowidx, tidx++, toString(found_objs));
      UpdateTableStringPtr(update, rowidx, tidx++, CString(buffer));
...
int CCommandStats::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
  ...
  CImageList *pImages =
    (((CTrackEditorApp *)AfxGetApp())->getSharedImageList());
