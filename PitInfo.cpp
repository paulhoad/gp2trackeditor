...
extern LPSTR toBinString(int data,int size);
...
#define UPDATE_TABLE_STRING_PTR(a,b,c,d) str =  d;UpdateTableStringPtr(a,b,c,CString(str));free(str);
...
void CPitInfo::UpdateInfo(BOOL update)
{
	...
	if (mytrack!=NULL)
	{
		  ...
		  if (mytrack->PitlaneSections)
		  {
		   ...
		   UPDATE_TABLE_STRING_PTR(update,i,tidx++,toString((int)(t->track->LengthToPitSectorEnd(t))));
...
void CPitInfo::OnImportTable() 
{
	...
	if (tabledata!=NULL)
	{
		int result = AfxMessageBox("About to Import Pit Data!, are you sure you want to do this",MB_YESNO);
		...
		if (result == IDYES)
		{
			if (tabledata->size() > mytrack->PitlaneSections->size())
			{
				//ignoreExtra = AfxMessageBox("More Table Data Than Track (Ignore Extra Table Data)?",MB_YESNO);
 				AfxMessageBox("More Table Data Than Track (Ignoring Extra Table Data, add section and reapply)?");
				ignoreExtra = TRUE;
			}
			else if (tabledata->size() < mytrack->PitlaneSections->size())
			{
				...
				AfxMessageBox("More pit Data Than Table (leaving end of pit unaltered!)");
			}
