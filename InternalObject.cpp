...
void InternalObjectCmd::LoadPropertyTable(CTrackPropertySheet *table)
{
	...
	for(int i=1;i<=numArgs;i++)
	{
		...
		switch(i)
		{
		   case 1: desc="Unk";break;
		   case 2: desc="Texture Id";break;
		   case 3: desc="Cmd";break;
		   case 4: desc="Texture Bending?";break;
		   case 5: desc="ncolumns of Texture";break;
		   case 6: desc="nrows of Texture";break;
		   case 7: desc="Unk";break;
		   case 8: desc="Unk";break;
		   case 9: desc="Unk";break;
		}
...
#define OFFSET_NODE(x,y) dat = (ObjectData*)objectData->elementAt(y); pos = dat->GetValue()- track->LargeNumber1.GetValue()-Offset+0x1020; buffer.Format("%s %d",x,pos); tree->insertInfoNode(offNode,buffer,TO_ID(IDB_OFFSET));
...
void InternalObject::LoadTreeTop(CTrackTree *tree,HTREEITEM node)
{
	...
	//SubTree = tree->insertInfoNode(node,CString("Textures (Old)"),TO_ID(IDB_JAMFILE));

	HTREEITEM offNode = SubTree = tree->insertInfoNode(node,CString("Offsets"),TO_ID(IDB_OFFSET));
	int pos=0;
	ObjectData *dat;

	//ObjectData *dat = (ObjectData*)objectData->elementAt(2); dat->GetValue()- track->LargeNumber1.GetValue()-Offset+0x1020; buffer.Format("Scale Section %d",pos); tree->insertInfoNode(offNode,buffer,TO_ID(IDB_OFFSET));
	...
	for(int i=0;i<numScale;i++)
	{
		...
		if (scaleData)
		{
			...
			SingleVariableObserver *itemData = new SingleVariableObserver(NULL,scaleData,"Scale Value","Scale Values Used for Object");			
	...
	for( i=0;i<Verticies;i++)
	{
	  ...
	  if ((X & 0xFF00)  == 0x8000)
	  {
		  ...
		  //ptA->y = Y;
	      ptA->z = (double)Z;	  
		  //buffer.Format("[%d] (A) %x %x %x  -> %x %x %x  (%d %d %d)",i,X,Y,Z,(int)ptA->x,(int)ptA->y,(int)ptA->z,(int)ptA->x,(int)ptA->y,(int)ptA->z);
		  ...
		  SingleVariableObserver *x_itemData = new SingleVariableObserver(NULL,Xdat,"X Data","X Scale Index/Previous Point");
		  ...
		  SingleVariableObserver *y_itemData = new SingleVariableObserver(NULL,Ydat,"Y Data","Y Scale Index/Previous Point");
		  ...
		  SingleVariableObserver *z_itemData = new SingleVariableObserver(NULL,Zdat,"Z Data","Z Scale Index/Previous Point");
	  ...
	  else
	  {
		  ...
		  //buffer.Format("[%d] %x %x %x  -> %x %x %x (%d %d %d) %d %d",i,Xid,Yid,Z,(int)ptA->x,(int)ptA->y,(int)ptA->z,(int)ptA->x,(int)ptA->y,(int)ptA->z,id,scaleIDx);
	...
	for(i=0;i<connections;i++)
	{
	  ...
	  //SingleVariableObserver * itemData = new SingleVariableObserver(NULL,PtPt,"From/To Vertex","(From + (256*To)) Index for Point");
	  SingleVertexObserver *itemData = new SingleVertexObserver(NULL,PtPt,"From/To Vertex","(From + (256*To)) Index for Point");
	  treeItemData->addElement(itemData);
	 ...
	 while(((idx-idxStart) < numBytes))
	 {
		...
		switch(unk1)
		{
		 ...
		 case 0x80:
		 {
			...
			HTREEITEM node = tree->insertInfoNode(textdata,msg,TO_ID(IDB_OBJECT));

			if (subObjects==NULL) subObjects = new Vector("SubObjects",__LINE__);
		...
		if (unk2 != -99) { CString msga; msga.Format("%x ",unk2);msg+=msga;}
		if (unk3 != -99) { CString msga; msga.Format("%x ",unk3);msg+=msga;}
		if (unk4 != -99) { CString msga; msga.Format("%x ",unk4);msg+=msga;}
		if (unk5 != -99) { CString msga; msga.Format("%x ",unk5);msg+=msga;}
		if (unk6 != -99) { CString msga; msga.Format("%x ",unk6);msg+=msga;}
		...
		HTREEITEM textureNode = tree->insertInfoNode(node,buffer,TO_ID(IDB_OFFSET));
		SingleVariableObserver *itemData =  new SingleVariableObserver(NULL,textureObj,"Texture Cmd Argument","Possible Texture Id");			
		treeItemData->addElement(itemData);
		tree->getTree()->SetItemData(textureNode,(DWORD)itemData);


		buffer.Format("Rotation?=%d",cmd);	    
		HTREEITEM textureRot = tree->insertInfoNode(node,buffer,TO_ID(IDB_OFFSET));
		SingleVariableObserver *itemDataRot =  new SingleVariableObserver(NULL,rotObj,"Texture Rotation","Possible Texture Rotation");			
		...
		if (unk2 != -99){
		 ...
		 HTREEITEM textureNode2 = tree->insertInfoNode(node,buffer,TO_ID(IDB_OFFSET));
		 SingleVariableObserver *itemData = new SingleVariableObserver(NULL,unk2Obj,"Texture Cmd Argument","Possible Repeat Number");			
		 treeItemData->addElement(itemData);
		...
		if (unk3 != -99){
		 ...
		 HTREEITEM textureNode2 = tree->insertInfoNode(node,buffer,TO_ID(IDB_OFFSET));
		 SingleVariableObserver *itemData = new SingleVariableObserver(NULL,unk3Obj,"Texture Cmd Argument","Possible Repeat Number");			
		 treeItemData->addElement(itemData);
		 tree->getTree()->SetItemData(textureNode2,(DWORD)itemData);			
		}
		//
		if (unk4 != -99){
		 ...
		 HTREEITEM textureNode2 = tree->insertInfoNode(node,buffer,TO_ID(IDB_OFFSET));
		 SingleVariableObserver *itemData = new SingleVariableObserver(NULL,unk4Obj,"Texture Cmd Argument","Possible Repeat Number");
		 treeItemData->addElement(itemData);
		 tree->getTree()->SetItemData(textureNode2,(DWORD)itemData);			
		}
		if (unk5 != -99){
		 ...
		 HTREEITEM textureNode2 = tree->insertInfoNode(node,buffer,TO_ID(IDB_OFFSET));
		 SingleVariableObserver *itemData = new SingleVariableObserver(NULL,unk5Obj,"Texture Cmd Argument","Possible Repeat Number");			
		 treeItemData->addElement(itemData);
		 tree->getTree()->SetItemData(textureNode2,(DWORD)itemData);			
		}
		if (unk6 != -99){
		 ...
		 HTREEITEM textureNode2 = tree->insertInfoNode(node,buffer,TO_ID(IDB_OFFSET));
		 SingleVariableObserver *itemData = new SingleVariableObserver(NULL,unk6Obj,"Texture Cmd Argument","Possible Repeat Number");			
		 treeItemData->addElement(itemData);
		 tree->getTree()->SetItemData(textureNode2,(DWORD)itemData);			
		}	
		...
		for(int i=0;i<vertexObjs.size();i++){
		 ...
		 HTREEITEM textureNode2 = tree->insertInfoNode(node,buffer,TO_ID(IDB_OFFSET));
		 SingleVariableObserver *itemData = new SingleVariableObserver(NULL,vertexObj,"Vertex","Vertex Number for polygon");			
		 treeItemData->addElement(itemData);
		 tree->getTree()->SetItemData(textureNode2,(DWORD)itemData);			
		}
	...
	for(i=0;i<numUnknowns;i++)
	{
		...
		if (scaleData)
		{
	        ...
	        HTREEITEM node = tree->insertInfoNode(unkParent,buffer,TO_ID(IDB_OFFSET));
			SingleVariableObserver *itemData = new SingleVariableObserver(NULL,scaleData,"Unk Value","Unk Values Used for Object");			
			treeItemData->addElement(itemData);
		    tree->getTree()->SetItemData(node,(DWORD)itemData);
		}
	}
...
void InternalObject::LoadTree(CTrackTree *tree,HTREEITEM node)
{
	...
	for(int count=32;count<listsize;count++)
	{
	   ...
	   switch(cmdType)
	   {
	      case 0x83:
			  {
				  InternalObjectCmd *IOcmd = new InternalObjectCmd(track,INTERAL_CMD_TEXTURE_83,8);
				  
				  // texture
				  int unk1      = ((ObjectData*)objectData->elementAt(count-2))->GetValue();
		          int textureid = ((ObjectData*)objectData->elementAt(count-1))->GetValue();
				  IOcmd->setNthArg(1,unk1);
				  IOcmd->setNthArg(2,textureid);
				  IOcmd->setNthArg(3,((ObjectData*)objectData->elementAt(count))->GetValue());
				  IOcmd->setNthArg(4,((ObjectData*)objectData->elementAt(count+1))->GetValue());
				  IOcmd->setNthArg(5,((ObjectData*)objectData->elementAt(count+2))->GetValue());
				  IOcmd->setNthArg(6,((ObjectData*)objectData->elementAt(count+3))->GetValue());
		  ...
		  case 0x03:
			  {
				  InternalObjectCmd *IOcmd = new InternalObjectCmd(track,INTERAL_CMD_TEXTURE_03,9);
				  // texture
				  int unk1      = ((ObjectData*)objectData->elementAt(count-2))->GetValue();
		          int textureid = ((ObjectData*)objectData->elementAt(count-1))->GetValue();
				  IOcmd->setNthArg(1,unk1);
...
void InternalObject::ReadObject(GPTrack *track)
{
	   ...
	   {
		   char msg[256];
		   wsprintf(msg,"Internal Object Error: Invalid Magic Number! 0x8002 at offset 0x%x id was 0x%x",offset,itemId);
...
#define EXPORT_HEX_ITEM(x,msg) {ObjectData *data = (ObjectData*)objectData->elementAt(x); data->exportHex(fp,(LPSTR)(LPCSTR)msg);}
#define EXPORT_DEC_ITEM(x,msg) {ObjectData *data = (ObjectData*)objectData->elementAt(x); data->export(fp,(LPSTR)(LPCSTR)msg);}
#define EXPORT_HEX_WORD(x) {ObjectData *data = (ObjectData*)objectData->elementAt(x); fprintf(fp,"%0x%x\t",(int)data->GetValue());}
#define GET_ITEM(x) (((ObjectData*)objectData->elementAt(x))->GetValue());
#define BRL   fprintf(fp,"\n");
#define HR(t) fprintf(fp,"##########%s ##########\n",(LPSTR)(t));

void InternalObject::Export()
{
	CString buffer;
	...
	BRL
	EXPORT_HEX_ITEM(1,"Internal Object Track Identifier")
	HR("Memory Offsets")
    EXPORT_HEX_ITEM(2,"Offset 1")
	int base  = GET_ITEM(2);
	int off2  = GET_ITEM(3);
	int off3  = GET_ITEM(4);
	int off4  = GET_ITEM(5);
	...
	BRL
	HR("Unknown 3 words")
	EXPORT_HEX_ITEM(11,"Unknown 1")
	EXPORT_HEX_ITEM(12,"Unknown 2")
	EXPORT_HEX_ITEM(13,"Unknown 3")
	BRL
	EXPORT_DEC_ITEM(14,"Number of Points")
	int size  = GET_ITEM(14);
	EXPORT_DEC_ITEM(15,"Number of Points*8")
	BRL

	HR("Unknown Unk words")
	int idx = 16;
	for(int i=0;i<17;i++)
	{
...
void ObjectData::export(FILE *fp,LPCSTR msg)
{
	fprintf(fp,"%s - %d\n",msg,GetValue());
}

#define UPDATE_OFFSET(x)  { ObjectData *dat = (ObjectData*)objectData->elementAt(x); dat->SetValue((dat->GetValue() - track->LargeNumber1.GetValue()-Offset+0x1020));}
...
BOOL InternalObject::ImportObject(CString filename)
{
	  ...
	  {
		CString str;
		str.Format("Not a GP2 Object! cannot include MagicNumber=%d elements=%d",itemId,listsize);
		AfxMessageBox(str);
		return FALSE;
	  }
...
#define UPDATE_NEW_OFFSET(x){ ObjectData *dat = (ObjectData*)objectData->elementAt(x);dat->setValue((dat->GetValue() + track->LargeNumber1.GetValue()+Offset-0x1020));}
...
void InternalObject::AdjustPointers()
{
	...
	ObjectData *dat = (ObjectData*)objectData->elementAt(2);
	long val =  (dat->GetValue() + track->LargeNumber1.GetValue()+Offset-0x1020-4);
	dat->SetValue(val);

	dat = (ObjectData*)objectData->elementAt(3);
	 val =  (dat->GetValue() + track->LargeNumber1.GetValue()+Offset-0x1020-4);
	dat->SetValue(val);
...
void InternalObject::Delete()
{
	...
	result  = AfxMessageBox("Shall I automatically renumber the track object descriptions\n to cater for the removed object",MB_YESNO);
	int result1 = AfxMessageBox("Shall I remove any track object description that reference the removed object?",MB_YESNO);
	int result2 = AfxMessageBox("Shall I also remove track and pit objects\n that reference these track object descriptions",MB_YESNO);
	...
	if (result == IDYES)
	{
		...
		{
			TrackObjectDefinition *def = (TrackObjectDefinition*)track->TrackObjectDefintions->elementAt(i);
			...
			if (def->id > 17)
			{
			  ...
			  else if (currentIndex == index && result1 == IDYES)
			  {
				...
				if (result2 == IDYES)
				{
			      ...
			      {
				     TrackSection *t = (TrackSection*)track->TrackSections->elementAt(j);
				  ...
				  for(j=0;j<track->PitlaneSections->size();j++)
			      {
				     TrackSection *t = (TrackSection*)track->PitlaneSections->elementAt(j);
				     if (t->containsCommand(0x80))
				     {
					    Vector *objs = t->getObjects();
					    for(int k=0;k<objs->size();k++)
		...
		CString msg;
		msg.Format("Removed %d Track Objects\nRemoved %d Pit Objects",trackObjectsRemoved,pitObjectsRemoved);
		AfxMessageBox(msg);		
	} // if remove
}
...
int InternalObject::CalculateDigitalSignature()
{
	...
	for(dat = (ObjectData*)objectData->elementAt(11);dat != last; dat = (ObjectData*)objectData->next(dat))
	{
		  //ObjectData *dat = (ObjectData*)objectData->elementAt(i);
		  AddSumValue(dat->GetValue());	
	}
...
CString *InternalObject::getLowerName()
{
...
void InternalObject::DrawVML(ofstream &file,double x,double y,double z,double angle,BOOL drawAxis)
{
	CreateObject();
		...
		for(int i=0;i<size;i++)
		{
			...
			for(i=0;i<ver->points;i++)
			{
				...
				{				 
				 ...
				 {
				  default:
				  case 0:vmlTexCoord2f(1.0f,0.0f);break;
				  case 1:vmlTexCoord2f(0.0f,0.0f);break;
				  case 2:
					  if (ver->points == 3)
					  {
					  ...
					  break;
				  case 3:vmlTexCoord2f(1.0f,1.0f);break;
				 }
				 cornerProcessed++;
