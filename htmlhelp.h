...
#define HH_SET_EXCLUSIVE_FILTER                                             \
  0x0019// set exclusive filtering method for untyped topics to be excluded \
    // from display
#define HH_SET_GUID \
  0x001A// For Microsoft Installer -- dwData is a pointer to the GUID string

#define HH_INTERNAL 0x00FF// Used internally.

#define HHWIN_PROP_ONTOP \
  (1 << 1)// Top-most window (not currently implemented)
...
#define HHWIN_PROP_TAB_BOOKMARKS \
  (1 << 12)// include bookmark tab in navigation pane
#define HHWIN_PROP_CHANGE_TITLE \
...
typedef struct tagHH_AKLINK
{
  ...
  LPCTSTR pszMsgText;// Message text to display in MessageBox if pszUrl is
...
typedef struct tagHH_WINTYPE
{
  ...
  DWORD
  fsWinProperties;// IN/OUT: Properties/attributes of the window (HHWIN_)
  ...
  BOOL
    fNotExpanded;// IN: TRUE/FALSE to contract or expand, OUT: current state
  ...
  BYTE tabOrder[HH_MAX_TABS + 1];// IN/OUT: tab order: Contents, Index, Search, History,
...
HWND WINAPI
  HtmlHelpW(HWND hwndCaller, LPCWSTR pszFile, UINT uCommand, DWORD dwData);
#ifdef UNICODE
