...
char *TrackObjectDefinition::getItemName()
{
...
int TrackObjectDefinition::read(GPTrack *track,int offset,int ListObjectOffset)
		{
		  int start  = offset;
		  id         = track->ReadPCByte(offset);offset+=1;
		  detail     = track->ReadPCByte(offset);offset+=1;
		  Unk1       = track->ReadPCWord(offset);offset+=2;
		  y          = track->ReadPCWord(offset);offset+=2;
		  angleX     = track->ReadPCWord(offset);offset+=2;
		  id2        = track->ReadPCWord(offset);offset+=2;
		  Unk2       = track->ReadPCWord(offset);offset+=2;
		  height     = track->ReadPCWord(offset);offset+=2;
		  angleY     = track->ReadPCWord(offset);offset+=2;
...
int TrackObjectDefinition::write(GPTrack *track,int offset)
		{
		  ...
		  track->WritePCByte(offset,id);offset+=1;
		  track->WritePCByte(offset,detail);offset+=1;
		  track->WritePCWord(offset,Unk1);offset+=2;
		  track->WritePCWord(offset,y);offset+=2;
		  track->WritePCWord(offset,angleX);offset+=2;
		  track->WritePCWord(offset,id2);offset+=2;
		  track->WritePCWord(offset,Unk2);offset+=2;
		  track->WritePCWord(offset,height);offset+=2;
		  track->WritePCWord(offset,angleY);offset+=2;
...
LPCSTR FindInternalObjectName(GPTrack *track,int id1,int id2)
{
	int internalId = id1-17;

	InternalObject *obj = (InternalObject*)track->InternalObjectOffsets->elementAt(internalId);
	if (obj)
	{
		return obj->getName();
	}
...
char * getDisplayObjectType(GPTrack *track,int id1,int id2)
          {
				...
				case 1:
                 {
                   switch(id2)
                   {
                   case 78: return "Sign Arrow Right";
                   case 79: return "Sign Arrow Staight Right";
				   case 80: return "Sign Arrow Left";
                   case 81: return "Sign Arrow Staight Left";
				   }
                 }break;
				case 9: return "Pit Hand";break;
				case 10: return "Pit Lollypop";break;
				case 11: return "Pit Monitor";break;
                case 12: return "Helments";break;
				case 0: return "Cars";break;
				
			     case 5:
                  {
                   switch(id2)
                   {
				   case 82: return "Column";
				   case 83: return "Yacht";
                   }
				   if (id2<65) return "Tyre";
				   else if (id2<110 && id2 > 83) return "Tree";
				   else if (id2>=100 && id2 <130) return "Bush";
				   else if (id2<226 && id2 >= 221) return "Monaco Tree";
			     ...
			     case 6: return "Marshall with black/white flag";
                 case 7: return "Marshall with blue flag";
				 case 14: return "Marshall with blue flag";
                 case 17: return "Large Pit Tower";
                 ...
                 case 13:
                 {
                   switch(id2)
                   {
                   case 75: return "100 Metre Sign";
                   case 76: return "200 Metre Sign";
                   case 77: return "300 Metre Sign";
                   }
                 }break;
             }
...
char * getNameLocal(GPTrack *track,int locator,int id,int id2)
{
...
char * TrackObjectDefinition::getName()
{  
...
int parseObjectName(char *name)
{
	...
	{
	   if (newObjectname.Find(ObjectLookUpTable[i].name)!=-1) return ObjectLookUpTable[i].id;
	}
	/*
...
int getOldDisplayIconType(GPTrack *track,int id,int id2)
{
			if (id < 17)
			{
			   ...
			   case 1:
               {
                   switch(id2)
                   {
                     case 78: return IDB_TURN_RIGHT;
                     case 79: return IDB_TURN_RIGHT;
                     case 80: return IDB_TURN_LEFT;
                     case 81: return IDB_TURN_LEFT;
                   } 
				   return IDB_UNK;
               }break;
		       ...
		       case 5:
               {
				   if (id2<65) return IDB_TYRE;
				   else if (id2<110 && id2 > 83) return IDB_TREE;
				   else if (id2>=100 && id2 <130) return IDB_BUSH;
				   else if (id2>=130 && id2 <220) return IDB_PALM;
				   else if (id2<238 && id2 >= 221) return IDB_TREE;
			  ...
			  case 6: return IDB_CHECKED_FLAG;
              case 7: return IDB_BLUE_FLAG;
              case 13:
               {
				   switch(id2)
				   {
			         case 75: return IDB_100_SIGN;
					 case 76: return IDB_200_SIGN;
					 case 77: return IDB_300_SIGN;
				   }
				   return IDB_100_SIGN;
               }break;
			  case 17: IDB_BUILDING;
			  }
...
void TrackObjectDefinition::DrawVML(ofstream &file,double x,double y,double z,double angle,BOOL drawAxis)
{
	     double side=2048.0f;
		 double s = side;
		 double s1 = 2048.0f;
