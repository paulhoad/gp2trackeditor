...
#define ENABLE_ARG(x) if (numArgs >= ##x){CEdit *ctrl = (CEdit*)GetDlgItem(IDC_ARG##x);if (ctrl)ctrl->SetReadOnly(FALSE);}else{CEdit *ctrl = (CEdit*)GetDlgItem(IDC_ARG##x);if (ctrl)ctrl->SetReadOnly(TRUE);}
...
extern CString getCmdHelp(int id);
