...
extern void
  drawSpiralPoly(Display *g, GPPolygon &poly, double cx, double cy, double angle1, double angle2, double r1, double r2, bool rev, int pts);

double
  GetTrackMoveOnPoint(double &nextxpos, double &nextypos, double stAngle, double angle, double length, double remainder)
{
  double nangle;
  ...
  if (beginOffset == 0 && sectorLength == length) {
    ...
    if (beginOffset + length < sectorLength) {
      ...
      if (beginOffset == 0) {
        ...
        // TRACE("(1)Texture map does not run a whole section from=%d len=%d ->
        // %d\n",beginOffset,length,sectorLength);
        drawSingleSectionSide(g, t1, beginOffset, length, side);
      } else {
        // the offset and the length need altering
        // TRACE("(2)Texture map does not run a whole section from=%d len=%d ->
        // %d\n",beginOffset,length,sectorLength);
        drawSingleSectionSide(g, t1, beginOffset, length, side);
      }
    } else {
      ...
      // TRACE("(3)Texture map does not fit whole section from=%d len=%d ->
      // %d\n",beginOffset,length,sectorLength);
...
void drawSingleSectionSide(Display *g, TrackSection *t1, int beginOffset, int length, int side)
{
  ...
  if ((beginOffset + length) < Length) {
    ...
    EndRightWidth =
      BeginRightWidth + ((beginOffset + length) * WidthRightDelta);
    EndLeftWall =
      BeginLeftWall + (((double)(beginOffset + length)) * WallLeftDelta);
    EndRightWall =
      BeginRightWall + (((double)(beginOffset + length)) * WallRightDelta);
  }

  if (beginOffset != 0) {
    double x1 = xpos;
    double y1 = ypos;
    double nangle = GetTrackMoveOnPoint(
      x1, y1, t1->getStartAngle(), t1->getAngle(), Length, beginOffset + 1);
  ...
  if (angle == 0) {
    // track parts
    TleftSideX =
      xpos + getLeftSideX(sectorStartAngle, WIDTH_TO_TRACKWIDTH(LeftWidth));
    TleftSideY =
      ypos + getLeftSideY(sectorStartAngle, WIDTH_TO_TRACKWIDTH(LeftWidth));
    ...
    TrightSideX =
      xpos + getRightSideX(sectorStartAngle, WIDTH_TO_TRACKWIDTH(RightWidth));
    TrightSideY =
      ypos + getRightSideY(sectorStartAngle, WIDTH_TO_TRACKWIDTH(RightWidth));
    TrightSideNX =
      xpos + nextxpos + getRightSideX(sectorEndAngle, WIDTH_TO_TRACKWIDTH(RightWidth));
    TrightSideNY =
      ypos + nextypos + getRightSideY(sectorEndAngle, WIDTH_TO_TRACKWIDTH(RightWidth));

    // walls parts
    WleftSideX =
      xpos + getLeftSideX(sectorStartAngle, WIDTH_TO_TRACKWIDTH(LeftWidth + BeginLeftWall));
    WleftSideY =
      ypos + getLeftSideY(sectorStartAngle, WIDTH_TO_TRACKWIDTH(LeftWidth + BeginLeftWall));
    ...
    WrightSideX =
      xpos + getRightSideX(sectorStartAngle, WIDTH_TO_TRACKWIDTH(RightWidth + BeginRightWall));
    WrightSideY =
      ypos + getRightSideY(sectorStartAngle, WIDTH_TO_TRACKWIDTH(RightWidth + BeginRightWall));
    WrightSideNX =
      xpos + nextxpos + getRightSideX(sectorEndAngle, WIDTH_TO_TRACKWIDTH(RightWidth + EndRightWall));
    WrightSideNY =
      ypos + nextypos + getRightSideY(sectorEndAngle, WIDTH_TO_TRACKWIDTH(RightWidth + EndRightWall));
  }

  if (side == LOC_FLOOR_LEFT) {
    if (angle < 0 || angle > 0) {
      double r1 =
        t1->getRadius() + WIDTH_TO_TRACKWIDTH(BeginLeftWidth + BeginLeftWall);
      double r2 =
        t1->getRadius() + WIDTH_TO_TRACKWIDTH(EndLeftWidth + EndLeftWall);
      double r3 = t1->getRadius() + WIDTH_TO_TRACKWIDTH(EndLeftWidth);
      double r4 = t1->getRadius() + WIDTH_TO_TRACKWIDTH(BeginLeftWidth);
      GPPolygon edge;
      edge.empty();
      drawSpiralPoly(
        g, edge, cx, cy, sectorStartAngle, sectorStartAngle + ((sectorEndAngle - sectorStartAngle) / 2.0), r1, r2, false, -1);
      drawSpiralPoly(
        g, edge, cx, cy, sectorStartAngle, sectorStartAngle + ((sectorEndAngle - sectorStartAngle) / 2.0), r4, r3, true, -1);
      ...
      // (ypos+nextypos),xpos,ypos,cx,cy,sectorStartAngle,TotalAngle,Radius+WIDTH_TO_TRACKWIDTH(LeftWidth+EleftWall),Radius+WIDTH_TO_TRACKWIDTH(LeftWidth));
    }
    // else if (angle > 0)
    //{

    // g->drawArcSpecial((xpos+nextxpos),
    // (ypos+nextypos),xpos,ypos,cx,cy,sectorStartAngle,TotalAngle,Radius+WIDTH_TO_TRACKWIDTH(LeftWidth),Radius+WIDTH_TO_TRACKWIDTH(LeftWidth+EleftWall));
    //}
    else {
      // left
    ...
    if (angle < 0 || angle > 0) {
      ...
      double r2 =
        t1->getRadius() - WIDTH_TO_TRACKWIDTH(EndRightWidth + EndRightWall);
      ...
      // drawSpiralPoly
      // g->drawArcSpecial((xpos+nextxpos),
      // (ypos+nextypos),xpos,ypos,cx,cy,sectorStartAngle,TotalAngle,Radius-WIDTH_TO_TRACKWIDTH(RightWidth),Radius-WIDTH_TO_TRACKWIDTH(RightWidth+ErightWall));
    }
    // else if (angle > 0)
    //{
    // g->drawArcSpecial((xpos+nextxpos),
    // (ypos+nextypos),xpos,ypos,cx,cy,sectorStartAngle,TotalAngle,Radius-WIDTH_TO_TRACKWIDTH(RightWidth+ErightWall),Radius-WIDTH_TO_TRACKWIDTH(RightWidth));
    //}
    else {
      // right
      GPPolygon *edge = new GPPolygon();
    ...
    if (angle < 0 || angle > 0) {
      ...
      // g->drawArcSpecial((xpos+nextxpos),
      // (ypos+nextypos),xpos,ypos,cx,cy,sectorStartAngle,TotalAngle,Radius+WIDTH_TO_TRACKWIDTH(RightWidth),Radius-WIDTH_TO_TRACKWIDTH(LeftWidth));
    }
    // else if (angle > 0)
    //{
    // g->drawArcSpecial((xpos+nextxpos),
    // (ypos+nextypos),xpos,ypos,cx,cy,sectorStartAngle,TotalAngle,Radius-WIDTH_TO_TRACKWIDTH(LeftWidth),Radius+WIDTH_TO_TRACKWIDTH(RightWidth));
    //}
    else {
      // road
      GPPolygon *edge = new GPPolygon();
    ...
    if (angle < 0 || angle > 0) {
      ...
      // g->drawArcSpecial((xpos+nextxpos),
      // (ypos+nextypos),xpos,ypos,cx,cy,sectorStartAngle,TotalAngle,Radius+WIDTH_TO_TRACKWIDTH(LeftWidth+EleftWall)+1,Radius+WIDTH_TO_TRACKWIDTH(LeftWidth+EleftWall));
    }
    // else if (angle > 0)
    //{
    // g->drawArcSpecial((xpos+nextxpos),
    // (ypos+nextypos),xpos,ypos,cx,cy,sectorStartAngle,TotalAngle,Radius+WIDTH_TO_TRACKWIDTH(LeftWidth+EleftWall),Radius+WIDTH_TO_TRACKWIDTH(LeftWidth+EleftWall)+1);
    //}
    else {
      // walls parts
      double OWleftSideX =
        xpos + getLeftSideX(sectorStartAngle, WIDTH_TO_TRACKWIDTH(LeftWidth + BeginLeftWall) + FENCE);
      double OWleftSideY =
        ypos + getLeftSideY(sectorStartAngle, WIDTH_TO_TRACKWIDTH(LeftWidth + BeginLeftWall) + FENCE);
      double OWleftSideNX =
        xpos + nextxpos + getLeftSideX(sectorEndAngle, WIDTH_TO_TRACKWIDTH(LeftWidth + EndLeftWall) + FENCE);
      double OWleftSideNY =
        ypos + nextypos + getLeftSideY(sectorEndAngle, WIDTH_TO_TRACKWIDTH(LeftWidth + EndLeftWall) + FENCE);

      // WrightSideX =
      // xpos+getRightSideX(sectorStartAngle,WIDTH_TO_TRACKWIDTH(RightWidth+rightWall));
      // WrightSideY =
      // ypos+getRightSideY(sectorStartAngle,WIDTH_TO_TRACKWIDTH(RightWidth+rightWall));
      // WrightSideNX =
      // xpos+nextxpos+getRightSideX(sectorEndAngle,WIDTH_TO_TRACKWIDTH(RightWidth+ErightWall));
      // WrightSideNY =
      // ypos+nextypos+getRightSideY(sectorEndAngle,WIDTH_TO_TRACKWIDTH(RightWidth+ErightWall));
      // left
      GPPolygon *edge = new GPPolygon();
      edge->empty();
      edge->addPoint(g->getScreenX(WleftSideX), g->getScreenY(WleftSideY));
    ...
    // g->drawArcSpecial((xpos+nextxpos),
    // (ypos+nextypos),xpos,ypos,cx,cy,sectorStartAngle,TotalAngle,Radius-WIDTH_TO_TRACKWIDTH(RightWidth+ErightWall),Radius-WIDTH_TO_TRACKWIDTH(RightWidth+ErightWall)+1);
    // else if (angle > 0)
    //{
    // g->drawArcSpecial((xpos+nextxpos),
    // (ypos+nextypos),xpos,ypos,cx,cy,sectorStartAngle,TotalAngle,Radius-WIDTH_TO_TRACKWIDTH(RightWidth+ErightWall)+1,Radius-WIDTH_TO_TRACKWIDTH(RightWidth+ErightWall));
    //}
    else {
      double OWrightSideX =
        xpos + getRightSideX(sectorStartAngle, WIDTH_TO_TRACKWIDTH(RightWidth + BeginRightWall) + FENCE);
      double OWrightSideY =
        ypos + getRightSideY(sectorStartAngle, WIDTH_TO_TRACKWIDTH(RightWidth + BeginRightWall) + FENCE);
      double OWrightSideNX =
        xpos + nextxpos + getRightSideX(sectorEndAngle, WIDTH_TO_TRACKWIDTH(RightWidth + EndRightWall) + FENCE);
      double OWrightSideNY =
        ypos + nextypos + getRightSideY(sectorEndAngle, WIDTH_TO_TRACKWIDTH(RightWidth + EndRightWall) + FENCE);
      ...
      edge->addPoint(g->getScreenX(OWrightSideNX),
        g->getScreenY(OWrightSideNY));
      edge->addPoint(g->getScreenX(WrightSideNX), g->getScreenY(WrightSideNY));
      edge->addPoint(g->getScreenX(WrightSideX), g->getScreenY(WrightSideY));
      if (t1->track->showFilledObjects) {
        edge->fillPolygon(g);
