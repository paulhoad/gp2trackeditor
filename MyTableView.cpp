...
static char lpszTabFilter[] = "Table Files (*.txt)|*.txt|Comma Separated Text(*.csv)|*.csv|All Files (*.*)|*.*|";
...
Vector * CMyTableView::ImportTableData()
{
	char fileName[256];

	strcpy(fileName,"*.csv");
	CFileDialog *fdlg = new CFileDialog(TRUE,"csv",(LPCSTR)&fileName,OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,lpszTabInFilter);
...
void CMyTableView::Export(CString filename)
{
    int i;
...
Vector *CMyTableView::Import(CString filename)
{
    int i;
...
int CMyTableView::DrawRow(CDC *pDC, int nItem)
{
     ...
     //Draw Normal and overlay icon
     pImageList = GetListCtrl().GetImageList(LVSIL_SMALL);  //assuming printing in report mode only
     if(pImageList)
     {
          UINT nOvlImageMask = lvi.state & LVIS_OVERLAYMASK;
