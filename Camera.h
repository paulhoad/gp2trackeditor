...
class Camera: public Observer , public CObject
{
  public:
	  int offset;
	  BOOL selected;
	  ...
	  Camera(GPTrack *mytrack,int myoffset):
		X(0),offset(myoffset), Observer(mytrack,t_Camera,NULL),selected(FALSE)
	  {}

	  Camera(GPTrack *mytrack,int number,int X,int Y,int Z1,int Z2,int SM,int ZF1,int ZF2,int VR1,int VR2,int UK1,int UK2):
		number(number),
	  X(X),Y(Y),Z1(Z1),Z2(Z2),SM(SM),ZF1(ZF1),ZF2(ZF2),VR1(VR1),VR2(VR2),UK1(UK1),UK2(UK2), Observer(mytrack,t_Camera,NULL)
	  {
			UK3 = 1;
			UK4 = 0;
			UK5 = 0;
      ...
      //void DrawCamera(Display *g,TrackSection *t,double inDistance,BOOL showBitmaps=TRUE);
	  void DrawCamera(Display *g,GPTrack *track);
	  ...
	  void setPerpendicularDistance(int d)
	  {
		  Y = d;
	  }
