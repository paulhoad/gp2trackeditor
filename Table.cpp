...
CTable::CTable(CWnd *parentView) : parent(parentView)
{
...
void CTable::empty()
{
  if (TableRows) TableRows->removeAllData();
  if (TableColumns) TableColumns->removeAllData();
}
...
static char lpszTabFilter[] =
  "Table Files (*.txt)|*.txt|Comma Separated (*.csv)|*.csv|All Files "
  "(*.*)|*.*|";
...
void CTable::OnMouseMove(UINT nFlags, CPoint point)
{
  ...
  if (draggingColumn && ColumnBeingDragged >= 0) {
    CTableColumn *col =
      (CTableColumn *)TableColumns->elementAt(ColumnBeingDragged);
    int xpos = (point.x + off.x);
