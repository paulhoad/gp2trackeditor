...
extern void LoadJamListBox(GPTrack *track,CListCtrl *list);

BOOL CTextureChooser::OnInitDialog() 
{
	CDialog::OnInitDialog();

	CheckDlgButton(IDC_JAM_PREVIEW,track->showJamPreviews ? BST_CHECKED : BST_UNCHECKED);
...
static void InsertString(CMyListCtrl *list,int row,int column,LPSTR name,BOOL update)
{
	LV_ITEM NameVal;
...
LPCSTR getSceneryLocation(int id)
{
	static char buffer[256];

	switch(id)
	{
	    case 104: wsprintf(buffer,"Verges Green Walls");break;
		case 112: wsprintf(buffer,"Scrubland");break;
		case 176: wsprintf(buffer,"Some Walls");break;
		case 186: wsprintf(buffer,"Track");break;
		case 194: wsprintf(buffer,"Trees and Bushes");break;
	    default: wsprintf(buffer,"Unknown Scenery Location (%d) (please report!)",id);break;
   }
		
   return buffer;
}

LPCSTR getTextureLocation(int id)
{
	static char buffer[256];

	switch(id)
	{
	  case 4: wsprintf(buffer,"Right Fence(%d)",id);break;
      case 5: wsprintf(buffer,"Left Fence(%d)",id);break;	  
	  case 8: wsprintf(buffer,"Right Verge(%d)",id);break;
	  case 9: wsprintf(buffer,"Left Verge(%d)",id);break;
	  case 10: wsprintf(buffer,"Road Surface(%d)",id);break;
	  case 11: wsprintf(buffer,"Ribbon1(%d)",id);break;
	  case 12: wsprintf(buffer,"Ribbon2(%d)",id);break;
	  case 13: wsprintf(buffer,"Ribbon3(%d)",id);break;
	  case 14: wsprintf(buffer,"Ribbon4(%d)",id);break;
      case 19: wsprintf(buffer,"Right Bank(%d)",id);break;
	  case 18: wsprintf(buffer,"Left Bank(%d)",id);break;

      default: wsprintf(buffer,"Unk(%d)",id);break;
   }
