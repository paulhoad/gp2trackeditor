...
public:
	...
	void setSection(CCLineSection *t,int idx=-1)
	{
	...
	void getSection(CCLineSection *t)
	{
		t->radius = m_ccrad;
	...
	BOOL EditCCSections(CCLineSection *t);

	CCLineSection *currentSection;

// Dialog Data
	//{{AFX_DATA(CCLineEditor)
	enum { IDD = CCLineChange };
	int		m_ccrad;
	int		m_cclen;
	int		m_cccmd;
	int		m_ccidx;
...
// Overrides
	...
	void OnMyChangeArgs(BOOL update=TRUE);
	void OnMyChangeRadiuscard(BOOL update=TRUE);
...
protected:
	...
	afx_msg void OnClickSpinrad(NMHDR* pNMHDR, LRESULT* pResult);
	afx_msg void OnChangeRadiuscard();
	afx_msg void OnRight();
	afx_msg void OnStraight();
	afx_msg void OnLeft();
	afx_msg void OnSetfocusCcrad();
	afx_msg void OnKillfocusCcrad();
	afx_msg void OnHelp();
	//}}AFX_MSG
	DECLARE_MESSAGE_MAP()
};
