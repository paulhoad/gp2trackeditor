...
void CTrackOpenGLView::OnInitialUpdate()
{
  ...
  for (int i = 0; i < mytrack->InternalObjectOffsets->size(); i++) {
    InternalObject *obj =
      (InternalObject *)mytrack->InternalObjectOffsets->elementAt(i);
    obj->UpdateStructure();
  }
...
void CTrackOpenGLView::getLookPoint()
{
  if (currentView == t_Air) {
    ...
    if (currentSector == NULL) {
      currentSector = (TrackSection *)mytrack->TrackSections->elementAt(
        currentSectorNumber);
    }
    ...
    gluLookAt(eyeX, eyeY, (eyeZ - car_height), lookX, lookY, (lookZ - car_height), 0.0f, 0.0f, -1.0f);
  } else {
    ...
    // TrackSection *t =
    // (TrackSection*)mytrack->TrackSections->elementAt(currentSectorNumber);
    if (currentSector == NULL) {
      currentSector = (TrackSection *)mytrack->TrackSections->elementAt(
        currentSectorNumber);
    }
    ...
    double distance_to_section_begin =
      mytrack->LengthToTrackSector(currentSector);
    ...
    if (currentCam != NULL) {
      CPosAngle *CameraPos =
        DistanceToPoint(mytrack, (currentCam->getDistance() - 16384));
      ...
      cameraFromZ =
        GET_GL_PT_Z(CameraPos->z + ((double)currentCam->getZ1() / 256.0));
      gluLookAt(cameraFromX, cameraFromY, -cameraFromZ, cameraLookX, cameraLookY, (cameraLookZ - car_height), 0.0f, 0.0f, -1.0f);
    }
  }
}
...
void CTrackOpenGLView::InitDisplayList(HDC hdc)
{
  ...
  if (mytrack->showScenery) DrawScenery(mytrack);
...
void CTrackOpenGLView::DrawRibbon3DGL(GPTrack *track, double xpos, double ypos, double zpos, double x1, double y1, double z1, double nx1, double ny1, double nz1, double x2, double y2, double z2, double nx2, double ny2, double nz2, int textureId)
{
  switch (textureId) {
  case 1:
    glColor3f(1.0f, 0.0f, 0.0f);
...
void TrackSection::drawScenery3DGL(CTrackOpenGLView *view)
{
  ...
  if (cmds != NULL && cmds->size() > 0) {
    ...
    for (int i = 0; i < cmds->size(); i++) {
      ...
      CreateRibbons(scenery, newx, newy, newz, 0, newangle, nx1, ny1, nz1, nx2, ny2, nz2, nx3, ny3, nz3, nx4, ny4, nz4, nx5, ny5, nz5, nx6, ny6, nz6, 4096, 0, 0, t_BothScenery);
...
void CTrackOpenGLView::LoadJamTexture(GPTrack *mytrack, CBitmap &output, int idx, int hres, int vres, int *width, int *height)
{
  ...
  *height = jam->getImageHeight(sidx);
  jam->LoadSingleBMPImage(temp, *width, *height, sidx);
  Mak256SquareImage(temp, output, *width, *height, hr, vr, td, td);
}

unsigned char *
  CTrackOpenGLView::TextureMap(CDC *pDC, CBitmap &cbmp, unsigned char *lpvBits, BOOL transparent)
{
  BITMAP bmp;
  cbmp.GetBitmap(&bmp);
  ...
  if (lpvBits == NULL) {
    ...
    // unsigned char *lpvBits = new unsigned char[bmp.bmWidth * bmp.bmHeight
    // *3];
    if (transparent) {
