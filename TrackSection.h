...
typedef enum { t_BothScenery, t_LeftScenery , t_RightScenery } e_ScenerySide;


class TrackSection: public Observer, public CObject
{
public:
	char *annotation;
	int  dataoffset;
	...
	TrackSection(Vector *own,GPTrack *track,int idx,int len,int ang,int dz,int wl, int wr,int rs,int cb,Vector *objs,int offset,double widthLeft,double widthRight, Vector *cmds,ObserverType section_type):
		...
		Observer(track,section_type,NULL),length(len),trackToWallWidthRight(wr),trackToWallWidthLeft(wl),
		subTree(NULL)
    {
		dataoffset = offset;
	...
	void setLeftToBank(int data)
	{
		if (data > 256) data = 256;
		trackToWallWidthLeft = data;
	}
	void setRightToBank(int data)
	{
		if (data > 256) data = 256;
	...
	BOOL isPitLaneEntrance();
	BOOL isPitLaneExit();
	BOOL IsRibbonOn(int ribbonid);
    ...
    double getX(double angleoffset)
    {
     ...
     double getScaledLength()
    {
        return (double)(2*length.GetValue());
    }

     double getY(double angleoffset)
    {
        double totalAngle = angleoffset; 
        double radangle   = DegToRad(totalAngle);
     ...
     int getLength()
    {
	 ...
	 double getTrueAltitude()
	 {
		 return TrueAltitude;
	 }

	 void setTrueAltitude(double alt)
	 {
		 TrueAltitude = alt;
	 }
	 ...
	 void setTrueAltitudeDelta(double alt)
	 {
		 AltitudeDelta = alt;
	 }
     ...
     void setEndAngle(double angle)
    {
	 ...
	 void setWidthLeft(int d)
	 {
		 startTrackWidthLeft = d;
	 }

	 void setWidthRight(int d)
	 {
    ...
    void drawTrack(Display *g,double angle,GPTrack *track,BOOL drawGray=FALSE);
	void HighlightSector(Display *g,GPTrack *track,double distanceIn,char *text);
	LinePt *drawTrackLine(Display *g,double angle,GPTrack *track);
	
	void drawSides(Display *g,double angle, GPTrack *track, TrackSection *t1,TrackSection *t2,TrackSection *prev);
	void drawTextureSides(Display *g,GPTrack *track);

	Primitive3D *drawTrack3D(Display *g,double angle,GPTrack *track,TrackSection *t1,TrackSection *t2);
	void drawRoadSign(Display *g);
	void drawCommands(Display *g);
	void drawTextureCommands(Display *g);
	void drawViewDistanceCommands(Display *g);
	void drawFlag(Display *g,BOOL right);
	void drawScenery(Display *g);
	Model3D *drawScenery3D(Display *g);
	void drawScenery3DGL(CTrackOpenGLView *view);

	BOOL isLeftKerb();
	BOOL isRightKerb();
	BOOL isLeftWallRemoved();
	BOOL isRightWallRemoved();
	BOOL isRoadSignLeft();
	BOOL isRoadSignRight();
	BOOL hasRoadSigns();
	BOOL isKerbLower();
	BOOL isLeftWallBridged();
	BOOL isRightWallBridged();

	void setLeftWallBridged(BOOL val);
	void setRightWallBridged(BOOL val);

	void setLeftKerb(BOOL on);
	void setRightKerb(BOOL on);
	void setLeftRoadSign(int val);
	void setRightRoadSign(int val);

	void GetBeginLeftWall(double &leftSideX,double &leftSideY);
	void GetBeginRightWall(double &leftSideX,double &leftSideY);
	void GetEndLeftWall(double &leftSideX,double &leftSideY);
	void GetEndRightWall(double &leftSideX,double &leftSideY);

	

	void CreateRibbons(TrackCmd *scenery,
					   ...
					   double LeftAngle,double RightAngle,e_ScenerySide Side);




	void AddGravelTrapLeft();
	void AddGravelTrapRight();
	void AddTyreWallLeft();
	void AddTypeWallRight();
	void AddTarmacLeft();
	void AddTarmacRight();
	void AddWallMarkingLeft(int id);
	void AddWallMarkingRight(int id);
	void AddTrackSideMarkingLeft(int id);
	void AddTrackSideMarkingRight(int id);
	void AddTrackSideMarking(int offset,int location,int length, int texture, int unk1,int unk2, int unk3, int rotation);
	void AddBlackFlagRight(int speed);
	void AddBlackFlagLeft(int speed);
	...
	void drawStartingLine(Display *g);

    void drawObjects(Display *g,double x,double y,double stAngle,int showBitmaps)
    {
	...
	void drawWalls(Display *g,double angle, GPTrack *track, TrackSection *t1,TrackSection *t2);

	void RemoveObject(TrackObject *obj);
	void RemoveCommand(TrackCmd *cmd);

	TrackCmd * getCommand(int cmd);
	Vector   * getCommands(int cmd);
	void RemoveCommand(int cmd);
	void InsertCommand(GPTrack *track,int cmd);
	
	BOOL containsCommand(int cmd)
	{
		if (getCommand(cmd)!= NULL) return TRUE;
		else return FALSE;
	}
	...
	TrackCmd * NthCommandOccurance(int cmd,int occurance)
	{
		Vector *cmds =getCommands(cmd);
	...
	void drawGrip(Display *g,double angle,GPTrack *track);
	...
	void addCommand(TrackCmd *cmd)
	{
		cmd->parent = this;
		TrackCommands->addElement(cmd);
	}

	BOOL isInside(CPoint begin,CPoint end);

	Vector *getCommandIterator();

	void InsertNew();
	void InsertNew(Observer *obj);
	void InsertCmdAt(int idx,int cmd);
	...
	CPoint TrackNextPoint(double xpos,double ypos,double angle,double stAnge,int inDistance);
	...
	HTREEITEM LoadTree(CTrackTree *mainTree,HTREEITEM HGP2Track,int index,BOOL TrackOrPit);
