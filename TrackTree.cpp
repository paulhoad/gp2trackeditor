...
extern int getOldDisplayIconType(GPTrack *track,int id1,int id2);
...
CString getTypeBLocation(int loc)
{
...
HTREEITEM CTrackTree::GetParent(HTREEITEM node)
{
...
#define  SINGLE_VAR(_parent,_buffer,_id,_var,_name,_desc){ HTREEITEM node = insertInfoNode(_parent,_buffer,TO_ID(_id)); SingleVariableObserver *data = new SingleVariableObserver(track,_var,_name,_desc); extraDataList->addElement(data); getTree()->SetItemData(node,(DWORD)data);}

#define  SINGLE_RGB_VAR(_parent,_buffer,_id,_var,_name,_desc){ HTREEITEM node = insertInfoNode(_parent,_buffer,TO_ID(_id)); SingleColorObserver *data = new SingleColorObserver(track,_var,_name,_desc); extraDataList->addElement(data); getTree()->SetItemData(node,(DWORD)data);}


char * getVariousString(int i)
{
	if (i >=50) {
		  if (!(i%2)) return "Bump Table Data (Position from start of track)";
	      else     return "Bump Table Data (value at above position)";
    }

	switch(i)
	{
	  case 11:return "Number of Laps";
	  case 14:return "% of cars doing one stop";
	  case 15:return "1 stopping cars stop from lap";
	  case 16:return "1 stopping cars stop during next n laps";
      case 22:return "% of cars doing two stop";
	  case 23:return "2 stopping cars stop 1 from lap";
	  case 24:return "2 stopping cars stop 1 during next n laps";
	  case 25:return "2 stopping cars stop 2 from lap";
	  case 26:return "2 stopping cars stop 2 during next n laps";
	  case 30:return "% of cars doing three stop";
	  case 31:return "3 stopping cars stop 1 from lap";
	  case 32:return "3 stopping cars stop 1 during next n laps";
	  case 33:return "3 stopping cars stop 2 from lap";
	  case 34:return "3 stopping cars stop 2 during next n laps";
	  case 35:return "3 stopping cars stop 3 from lap";
	  case 36:return "3 stopping cars stop 3 during next n laps";
...
char * GetPolePositionSide(int value)
{
	if (value == 768) return "Left";
	else if (value == 64768) return "Right";
	else return "Unable to Determine Pole Position Side";
}

char * GetPitSide(int value)
{
	unsigned char bits = 0x0F & LOBYTE(value);
	if (bits == 10) return "Left";
	else if (bits == 0) return "Right";
	else return "Unable to Determine Pit Lane Side";
}

CString GetKerbColors(int val)
{
	switch (val)
	{
	case 0: return "Black";
	...
	case 41: return "Red";
	case 57: return "Yellow";
	case 156: return "Blue";
	case 106: return "Green";
	}
	return "Unknown Color Number";
}

HTREEITEM CTrackTree::insertTrack(GPTrack *track)
{	
	CString buffer;
	HTREEITEM TrackRoot;
	...
	if (m_TrackTree)
	{
		...
		// GP2INFO
		HTREEITEM HGP2INFO = insertInfoNode(TrackRoot,CString("Track Infomation"),TO_ID(IDB_FILE));
		...
		//Statics
		HTREEITEM HPTrackStats = insertInfoNode(TrackRoot,CString("Track Config"),TO_ID(IDB_OFFSET));
		char *str = track->getTrackSectionsStr();
		buffer.Format("Track Sections=%s",str);
		free(str);
		HTREEITEM HPTrackDataStats = insertInfoNode(HPTrackStats,buffer,TO_ID(IDB_TRACK));
		...
		buffer.Format("Track Start Height (Delta)=%d",track->TrackStartClimb.GetValue());
		...
		buffer.Format("Begin Right Bank =%d",track->BeginTrackWidthRight.GetValue());
		...
		buffer.Format("Pole Position Side=%s",GetPolePositionSide(track->TrackUnknown1.GetValue()));
		...
		buffer.Format("PitLane Side=%s (+0xC5 code related?)val=%d",GetPitSide(track->C5Code.GetValue()), track->C5Code.GetValue());
		CString msg;
		int rightvalue = 0xFFF0 & track->C5Code.GetValue();
		int leftvalue = (0xFFF0 & track->C5Code.GetValue()) | 10;
		msg.Format("Command Value affects C5 argument number - PitLane Side Left=%d/Right=%d",leftvalue,rightvalue);

		SINGLE_VAR(HPTrackDataStats,buffer,IDB_PIT,&track->C5Code,"PitLane Side and (C5 Code)",(LPSTR)(LPCSTR)msg)

		buffer.Format("Offset to starting light object=%x",track->JamFileUnknown.GetValue());
		...
		{
			buffer.Format("(Untextured Kerb Colors ?=%d  Color=%s",track->UnknownNumbers[k].GetValue(),GetKerbColors(track->UnknownNumbers[k].GetValue()));
	    ...
	    HTREEITEM HPPitDataStats = insertInfoNode(HPTrackStats,buffer,TO_ID(IDB_PITLANE));
		free(str);

	    buffer.Format("PitLaneEntryIndex Section %d",track->PitLaneEntryIndex);
		insertInfoNode(HPPitDataStats,buffer,TO_ID(IDB_PITLANE));
		//SINGLE_VAR(HPPitDataStats,buffer,IDB_PITLANE,track->PitLaneEntryIndex,"PitLaneEntryIndex","Track Section Where Pit Lane Entrance command can be found")


		buffer.Format("PitLaneEntryIndex Section %d",track->PitLaneExitIndex);
		insertInfoNode(HPPitDataStats,buffer,TO_ID(IDB_PITLANE));
		//SINGLE_VAR(HPPitDataStats,buffer,IDB_PITLANE,track->PitLaneExitIndex,"PitLaneExitIndex","Track Section Where Pit Lane Exit command can be found")
		...
		HTREEITEM HGP2CCStats = insertInfoNode(HPTrackStats,buffer,TO_ID(IDB_CC_LINE));
		free(str);


		buffer.Format("CCLine StartX %d",(int)track->CCLineStartX.GetValue());
		SINGLE_VAR(HGP2CCStats,buffer,IDB_OFFSET,&track->CCLineStartX,"CCLineStartX","X Coordinate for beginning of cc line pushes the cc line to left/right at beginning")

		buffer.Format("CCLine StartX (High)%d",(int)track->CCLineStartXHigh.GetValue());
		...
		for(int j=0;j<track->VariousDataCount;j++)
		{
		 ...
		 char *vstr  = strdup(getVariousString(j));
		...
		HTREEITEM carsetup = insertInfoNode(HPTrackStats,buffer,TO_ID(IDB_OFFSET));
		for(j=0;j<19;j++)
		{
		...
		HTREEITEM HGPOffsets = insertInfoNode(TrackRoot,CString("Track Offsets"),TO_ID(IDB_FILE));
		...
		HTREEITEM HGP2CCLine = insertInfoNode(TrackRoot,buffer,TO_ID(IDB_CC_LINE));
		free(str);

		
		int ccsections = track->CCLineSections->size();
		for(i=0;i<ccsections;i++)
        {
		   ...
		   HTREEITEM HGP2TrackSection = insertInfoNode(HGP2CCLine,buffer,TO_ID(id));
		...
		HTREEITEM HGP2Objects = insertInfoNode(TrackRoot,buffer,TO_ID(IDB_OBJECT));
		free(str);

		int numberOfObjectDefs = track->TrackObjectDefintions->size();
	    for(i=0;i<numberOfObjectDefs;i++)
        {
           TrackObjectDefinition *t = (TrackObjectDefinition*)track->TrackObjectDefintions->elementAt(i);
		   buffer.Format("[%d](%s)  Offset=%d DFC=%d",i,t->getName(),t->getLocator(),t->getDistance());
		   int id = t->getDisplayIconType();
		   HTREEITEM HGP2TrackSection = insertInfoNode(HGP2Objects,buffer,TO_ID(id));
		   getTree()->SetItemData(HGP2TrackSection,(DWORD)t);
		   t->setTreeNode(HGP2TrackSection);
		...
		HTREEITEM HGP2IntObjects = insertInfoNode(TrackRoot,buffer,TO_ID(IDB_OBJECT_INTERNAL));
	    for(i=0;i<numberOfInternalObjects;i++)
        {
           InternalObject *t = (InternalObject*)track->InternalObjectOffsets->elementAt(i);
		   
	   
		   buffer.Format("[%d] id1=%d %s 0x%x size=%d Points=%d",i,i+17,t->getName(),t->getOffset(),t->getSize(),t->getNumPoints());
		   HTREEITEM HP2Obj = insertInfoNode(HGP2IntObjects,buffer,TO_ID(getOldDisplayIconType(track,i+17,0)));
		   getTree()->SetItemData(HP2Obj,(DWORD)t);
		   //t->LoadTreeTop(this,HP2Obj);
		   t->setTreeNode(HP2Obj);
		   t->SubTree = insertInfoNode(HP2Obj,CString("Undiscovered"),TO_ID(IDB_OFFSET));
		}
		...
		HTREEITEM HGP2Cameras = insertInfoNode(TrackRoot,buffer,TO_ID(IDB_CAMERA));
		...
		if (track->carsetup)
		{
			...
			HTREEITEM HGP2FWingSetup = insertInfoNode(HGP2Setup,buffer,TO_ID(IDB_FRONT_WING));
			getTree()->SetItemData(HGP2FWingSetup,(DWORD)track->carsetup);

			buffer.Format("CC Rear Wing=%d",track->carsetup->CCRearWing);
			HTREEITEM HGP2RWingSetup = insertInfoNode(HGP2Setup,buffer,TO_ID(IDB_REAR_WING));
			...
			buffer.Format("Tyre Compound =%c val=%d",(char)((track->carsetup->CCTyreType)+13),track->carsetup->CCTyreType);
			...
			buffer.Format("CC Fuel Consumption? =%d",track->carsetup->CCFuelConsumption);
		...
		if (track->JamFiles!=NULL)
		{
		 ...
		 HTREEITEM HGP2JamFiles = insertInfoNode(TrackRoot,buffer,TO_ID(IDB_JAMFILE));
	     for(i=0;i<numberOfJamFiles;i++)
         {
		...
		HTREEITEM HPMisc = insertInfoNode(TrackRoot,CString("Misc Config"),TO_ID(IDB_OFFSET));
		...
		buffer.Format("UnderLayBitmapWidth %d",(int)track->UnderLayBitmapWidth.GetValue());
		SINGLE_VAR(HPMisc,buffer,IDB_OFFSET,&track->UnderLayBitmapWidth,"UnderLayBitmapWidth","Width underlay bitmap")
		buffer.Format("UnderLayBitmapHeight %d",(int)track->UnderLayBitmapHeight.GetValue());
...
TrackObjectDefinition * GPTrack::FindObjectDefinition(int Offset)
{
...
HTREEITEM CTrackTree::insertInfoNode(HTREEITEM parent, CString &title,int bitmap)
{
	if (m_TrackTree)
	{
		TV_INSERTSTRUCT tvstruct;
...
BEGIN_MESSAGE_MAP(CTrackTree, CSizingControlBar)
	...
	ON_UPDATE_COMMAND_UI(ID_RIGHTPOP_MOVETONEXTSECTION, OnUpdateRightpopMovetonextsection)
	ON_UPDATE_COMMAND_UI(ID_RIGHTPOP_MOVETOPREVIOUSSECTION, OnUpdateRightpopMovetoprevioussection)
...
void CTrackTree::OnPaint() 
{
	...
	if (m_TrackTree && ImagesNotLoaded)
		   LoadImages();
...
void CTrackTree::OnAddJamFile()
{
	...
	if (selected != NULL)
	{
		...
		{
			AfxMessageBox("Which Track? No Selection (select jamfile or tracksection");
		}
	}
	else
	{
	...
	if (selected != NULL)
	{
		...
		{	
			...
			if (CommandsSection!=NULL)
			{
				...
				{
					AfxMessageBox("Cannot move the first command up select second commands");
				}			
			}
			else
			{
	...
	if (selected != NULL)
	{
		...
		{	
			...
			if (CommandsSection!=NULL)
			{
				...
				{
					AfxMessageBox("Cannot move the last command down select second to last commands");
				}			
			}
			else
			{
...
void CTrackTree::OnExportObject() 
{
	...
	if (selected != NULL)
	{
		...
		{	
			...
			CFileDialog *fdlg = new CFileDialog(FALSE,"gpo",(LPCSTR)&fileName,OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,lpszObjFilter);
...
int CTrackTree::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	if (!m_TrackTree.CreateEx(WS_EX_CLIENTEDGE,"SysTreeView32","",TVS_HASLINES  | TVS_HASBUTTONS | TVS_SHOWSELALWAYS | TVS_DISABLEDRAGDROP | WS_VISIBLE | WS_CHILD | WS_BORDER ,0,0,cx,cy,GetSafeHwnd(),(HMENU)IDC_TRACKTREE))
       return -1;
...
void CTrackTree::Reposition(CRect rc)
{
    ...
    m_TrackTree.SetWindowPos(NULL,rc.left + border, rc.top + border,
	...
	if (selected != NULL)
	{
		...
		{					
			...
			if (CommandsSection!=NULL)
			{
				...
				{
				TrackSection * nextSection = (TrackSection*)sections->elementAt(newIndex);
				Vector *cmds  = CommandsSection->getCommands();
				Vector *ncmds = nextSection->getCommands();
				if (cmds && cmd && ncmds)
				{
